...
RSpec.describe "将棋ウォーズ棋譜検索", type: :system do
  ...
  describe "index" do
    xit "トップ" do
      visit "http://localhost:4000/swars/battles"
    ...
    xit "アプリ起動できるブックマーク可能なページに飛ぶ" do
      visit "http://localhost:4000/swars/battles?query=devuser1"
    ...
    # it "連打対策" do
    #   visit "http://localhost:4000/swars/battles?query=devuser1&raise_duplicate_key_error=1"
    #   expect(page).to have_content "データ収集中なのであと15秒ぐらいしてからお試しください"
    #   doc_image
    # end

    xit "仕掛けの局面表示" do
      visit "http://localhost:4000/swars/battles?query=devuser1&display_key=critical"
      assert { find(".radio.is-primary").text === "仕掛け" }
      doc_image
    end

    xit "終了の局面表示" do
      visit "http://localhost:4000/swars/battles?query=devuser1&display_key=last"
    ...
    xit "modal_id の指定があるときモーダルが出て閉じたとき一覧にも1件表示されている" do
      visit "http://localhost:4000/swars/battles?modal_id=#{record.to_param}"
      find(".delete").click
      page.refresh
