...
RSpec.describe "エラー処理", type: :system do
  it "ページが見つかりません" do
    visit2 "/lab/swars/xxx"
    assert_text "ページが見つからないか権限がありません"
  end

  it "指定のメッセージを出す" do
    primary_error_message = SecureRandom.hex
    visit2 "/lab/chore/status_code", status_code: 404, primary_error_message: primary_error_message
    assert_selector(:element, text: primary_error_message, exact_text: true)
  end

  it "権限がありません" do
    visit2 "/lab/chore/status_code", status_code: 403
    assert_selector(:element, text: "ログインする", exact_text: true)
  end

  it "ぶっこわれました" do
    visit2 "/lab/chore/status_code", status_code: 500
