...
RSpec.describe "チャット_発言履歴の復帰と遡り", type: :system, share_board_spec: true do
  ...
  it "完全に抜けた状態から再度入室すると発言履歴がある" do
    ...
    visit_room(user_name: :alice, mh_per_page: 1)   # 入室する
  ...
  it "初回のスクロール位置はいちばん下になっている" do
    ...
    visit_room(user_name: :alice, mh_per_page: 50)  # 1回で50件読む
    assert_var("ml_count", 0)                                 # 件数は 0
    chat_modal_open do
  ...
  it "遡って昔の発言を参照する" do
    chat_message_setup(100)                                               # 100件用意する
    visit_room(user_name: :alice, mh_per_page: 40)  # 1回で40件読む (つまり3回のアクセスが必要)
