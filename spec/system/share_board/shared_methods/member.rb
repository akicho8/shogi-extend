module SharedMethods
  # メンバーの状態
  def assert_member_status(user_name, klass)
    assert_selector(".SbMemberList .#{klass} .user_name", text: user_name, exact_text: true)
  end
  ...
  def assert_member_has_text(user_name, text)
    el = find(".SbMemberOne .user_name", text: user_name, exact_text: true).find(:xpath, "..")
