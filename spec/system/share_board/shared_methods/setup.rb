module SharedMethods
  ...
  def visit_base_default_options
    {
      ...
      :dual_role_enabled_p             => false,                        # 自分vs自分禁止 (対局設定で対戦相手がいない場合はバリデーションする)
      ...
      :ranking_auto_open_p               => false,                        # 対局終了後にランキングを表示する
    }
  end
  ...
  def assert_room_created
    assert_var("cable_p", "true", wait: room_create_delay + Capybara.default_max_wait_time) # wait は room_create_delay より大きくすること
  end
