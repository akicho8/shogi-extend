...
RSpec.describe __FILE__, type: :system, share_board_spec: true do
  ...
  it "完全に抜けた状態から再度入室すると発言履歴がある" do
    ...
    visit_room(user_name: :a, mh_per_page: 1)   # 入室する
  ...
  it "初回のスクロール位置はいちばん下になっている" do
    ...
    visit_room(user_name: :a, mh_per_page: 50)  # 1回で50件読む
    assert_var("ml_count", 0)                                 # 件数は 0
    chat_modal_open do
      assert_message_received_o("(content:99)")                           # 50..99 は有り
      assert_message_received_x("(content:49)")                           #  0..49 は無し
      assert { chat_scroll_ratio.round == 1 }                                 # 一番下までスクロールしている
  ...
  it "よそ見から復帰したとき(タブを切り替えて戻ったとき)に最新にするためリロードする" do
    ...
    if false
      window_b { }                                                          # タブを切り替えて戻る (headless だと visibilitychange が効かないため)
    else
      window_a { find(".ChatModal .mh_reload").click }                     # デバッグ用の「よそ見から復帰」をクリックすると
    end
    window_a do
  ...
  it "遡って昔の発言を参照する" do
    chat_message_setup(100)                                               # 100件用意する
    visit_room(user_name: :a, mh_per_page: 40)  # 1回で40件読む (つまり3回のアクセスが必要)
  ...
  it "入室と退室のタイミングで履歴を消す" do
    ...
    room_menu_open_and_input(:test_room, :a)                        # 入室すると
