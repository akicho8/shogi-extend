  ...
  RSpec.describe PlayerIdSuggestion, type: :model, swars_spec: true do
    ...
    describe "アルファベットを含む半角を3文字以上入力したのでサジェクションが発動する" do
      ...
      it "複数人マッチした場合" do
        ...
        assert { PlayerIdSuggestion.new("alice").message == "alice に近い人は2人います。もしかして alice2 ですか？" }
      end

      it "マッチしない" do
        assert { PlayerIdSuggestion.new("alice").message == "alice に近い人はいません。正確に入力してください" }
      end

      it "部分一致とする" do
        User.create!(user_key: "alice")
        assert { PlayerIdSuggestion.new("lic").message == "lic に近い人は1人います。もしかして alice ですか？" }
      end
    end
