...
module Swars
  RSpec.describe MembershipMedalInfo, type: :model, swars_spec: true do
    ...
    describe "切断マン" do
      ...
      it "works" do
        assert2 { case1(13) != [:切断マン, "悔しかったので切断した"] }
        assert2 { case1(14) == [:切断マン, "悔しかったので切断した"] }
      end
    end
    ...
    describe "1手詰じらしマン" do
      ...
      it "works" do
        ...
        assert2 { case1 == [:"1手詰じらしマン", "1手詰を9分58秒焦らして歪んだ優越感に浸った"] }
      end
    end
    ...
    describe "長考" do
      ...
      it "works" do
        assert2 { test(2.5, :lose) == [:長考マン, "考えすぎて負けた。ちなみにいちばん長かったのは2分30秒"] }
        assert2 { test(3.0, :win)  == [:大長考マン, "対局放棄と受け取られかねない3分の長考をした"] }
        assert2 { test(3.0, :lose) == [:大長考負けマン, "対局放棄と受け取られかねない3分の長考をしたあげく負けた"] }
      end
    end
    ...
    describe "ただの千日手" do
      ...
      it "works" do
        ...
        assert2 { test(4*4) == [:ただの千日手, "千日手"]             }
        assert2 { test(3*4) == [:開幕千日手, "最初から千日手にした"] }
      end
    end
    ...
    describe "運営支えマン" do
      ...
      it "works" do
        ...
        assert2 { test(20) == [:運営支えマン, "将棋ウォーズの運営を支える力がある"] }
      end
    end
    ...
    describe "段級差" do
      ...
      it "全パターン" do
        ...
        assert2 {  case1("30級", "1級") == ["恐怖の級位者として無双した", "達成率をがっつり奪われた(ように感じるが実際はそんな減ってない)"] }
        assert2 {  case1("12級", "1級") == ["恐怖の級位者として無双した", "達成率をがっつり奪われた(ように感じるが実際はそんな減ってない)"] }
        assert2 {  case1("11級", "1級") == ["恐怖の級位者として無双した", "達成率をがっつり奪われた(ように感じるが実際はそんな減ってない)"] }
        assert2 {  case1("10級", "1級") == ["9つも格上の人を倒した", "9つも格下の人に勝って当然なのに負けた"] }
        assert2 {  case1( "9級", "1級") == ["8つも格上の人を倒した", "8つも格下の人に勝って当然なのに負けた"] }
        assert2 {  case1( "8級", "1級") == ["7つも格上の人を倒した", "7つも格下の人に勝って当然なのに負けた"] }
        assert2 {  case1( "7級", "1級") == ["6つも格上の人を倒した", "6つも格下の人に勝って当然なのに負けた"] }
        assert2 {  case1( "6級", "1級") == ["5つも格上の人を倒した", "5つも格下の人に勝って当然なのに負けた"] }
        assert2 {  case1( "5級", "1級") == ["4つも格上の人を倒した", "4つも格下の人に勝って当然なのに負けた"] }
        assert2 {  case1( "4級", "1級") == ["3つも格上の人を倒した", "3つも格下の人に勝って当然なのに負けた"] }
        assert2 {  case1( "3級", "1級") == ["2つも格上の人を倒した", "2つも格下の人に勝って当然なのに負けた"] }
        assert2 {  case1( "2級", "1級") == ["格上のライバルを倒した", "1つ格下の人に負けた"] }
        assert2 {  case1( "1級", "1級") == ["同じ棋力のライバルに勝った", "同じ棋力のライバルに負けた"] }
        assert2 {  case1("初段", "1級") == ["格下の人に着実に勝った", "格上のライバルにやっぱり負けた"] }
        assert2 {  case1("二段", "1級") == ["格下の人に当然のように勝った", "格上の人に当然のように負けた"] }
        assert2 {  case1("三段", "1級") == ["格下の人に当然のように勝った", "格上の人に当然のように負けた"] }
        assert2 {  case1("四段", "1級") == ["格下の人に当然のように勝った", "格上の人に当然のように負けた"] }
        assert2 {  case1("五段", "1級") == ["格下の人に当然のように勝った", "格上の人に当然のように負けた"] }
        assert2 {  case1("六段", "1級") == ["格下の人に当然のように勝った", "格上の人に当然のように負けた"] }
        assert2 {  case1("七段", "1級") == ["格下の人に当然のように勝った", "格上の人に当然のように負けた"] }
        assert2 {  case1("八段", "1級") == ["格下の人に当然のように勝った", "格上の人に当然のように負けた"] }
        assert2 {  case1("九段", "1級") == ["格下の人に当然のように勝った", "格上の人に当然のように負けた"] }
        assert2 {  case1("十段", "1級") == ["格下の人に当然のように勝った", "格上の人に当然のように負けた"] }
      end
    end
  end
end
