...
module Swars
  RSpec.describe MembershipMedalInfo, type: :model, swars_spec: true do
    ...
    describe "段級差" do
      ...
      it "全パターン" do
        test1("30級", "1級") # => ["恐怖の級位者として無双した", "達成率をがっつり奪われたように感じるが実際はそんな減ってない"]
        test1("12級", "1級") # => ["恐怖の級位者として無双した", "達成率をがっつり奪われたように感じるが実際はそんな減ってない"]
        test1("11級", "1級") # => ["恐怖の級位者として無双した", "達成率をがっつり奪われたように感じるが実際はそんな減ってない"]
        ...
        assert {  test1("30級", "1級") == ["恐怖の級位者として無双した", "達成率をがっつり奪われたように感じるが実際はそんな減ってない"] }
        assert {  test1("12級", "1級") == ["恐怖の級位者として無双した", "達成率をがっつり奪われたように感じるが実際はそんな減ってない"] }
        assert {  test1("11級", "1級") == ["恐怖の級位者として無双した", "達成率をがっつり奪われたように感じるが実際はそんな減ってない"] }
