...
module Swars
  RSpec.describe MembershipMedalInfo, type: :model, swars_spec: true do
    ...
    describe "1手詰じらしマン" do
      ...
      it "works" do
        test1                   # => [:"1手詰じらしマン", "1手詰を9分58秒焦らし歪んだ優越感に浸った"]
        assert { test1 == [:"1手詰じらしマン", "1手詰を9分58秒焦らし歪んだ優越感に浸った"] }
      end
    end
    ...
    describe "段級差" do
      ...
      it "全パターン" do
        test1("30級", "1級") # => ["恐怖の級位者として無双した", "達成率をがっつり奪われた(ように感じるが実際はそんな減ってない)"]
        test1("12級", "1級") # => ["恐怖の級位者として無双した", "達成率をがっつり奪われた(ように感じるが実際はそんな減ってない)"]
        test1("11級", "1級") # => ["恐怖の級位者として無双した", "達成率をがっつり奪われた(ように感じるが実際はそんな減ってない)"]
        ...
        assert {  test1("30級", "1級") == ["恐怖の級位者として無双した", "達成率をがっつり奪われた(ように感じるが実際はそんな減ってない)"] }
        assert {  test1("12級", "1級") == ["恐怖の級位者として無双した", "達成率をがっつり奪われた(ように感じるが実際はそんな減ってない)"] }
        assert {  test1("11級", "1級") == ["恐怖の級位者として無双した", "達成率をがっつり奪われた(ように感じるが実際はそんな減ってない)"] }
