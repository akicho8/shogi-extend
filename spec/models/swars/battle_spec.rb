...
module Swars
  RSpec.describe Battle, type: :model, swars_spec: true do
    ...
    it "KIFの直リンクがモデルから取れる" do
      assert2 { record.kif_url == "http://localhost:3000/w/alice-bob-20000101_000000.kif" }
    end
    ...
    describe "Twitterカード" do
      describe "to_twitter_card_params" do
        ...
        it "works" do
          assert2 { value[:title]       == "将棋ウォーズ(10分) user1 30級 vs user2 30級"              }
          assert2 { value[:url]         == nil                                                        }
          assert2 { value[:image]       == "http://localhost:3000/w/alice-bob-20000101_000000.png?turn=5&viewpoint=black" }
          assert2 { value[:description] == "新嬉野流 vs 2手目△３ニ飛戦法"                                   }
        end
        it "turnを変更できる" do
          assert2 { record.to_twitter_card_params(turn: 0)[:image].include?("turn=0") }
        end
      end

      it "title" do
        assert2 { record.title == "user1 30級 vs user2 30級" }
      end

      it "description" do
        assert2 { record.description == "新嬉野流 vs 2手目△３ニ飛戦法" }
      end
    end

    describe "時間チャート" do
      it "raw_sec_list_all: 消費時間" do
        assert2 { record.raw_sec_list_all == [1, 3, 5, 7, 2] }
      end
      ...
      it "time_chart_params: chart.jsに渡すデータがある" do
        assert2 { record.time_chart_params[:tcv_normal].has_key?(:datasets) }
        assert2 { record.time_chart_params[:tcv_accretion].has_key?(:datasets) }
      end
      ...
      describe "投了" do
        ...
        it "後手は時間切れでないので放置時間は無し" do
          assert2 { record.memberships[1].leave_alone_seconds == nil }
        end
        it "それぞれの最大考慮時間が取れる" do
          assert2 { record.memberships[0].think_max == 5 }
          assert2 { record.memberships[1].think_max == 7 }
        end
        it "ラベルの最大" do
          assert2 { record.time_chart_label_max == 6 }
        end
        it "それぞれの時間チャートデータが取れる" do
          assert2 { record.time_chart_xy_list2(:black, false) == [{x: 0, y: nil}, {x: 1, y: 1}, {x: 2, y: nil}, {x: 3, y: 5}, {x: 4, y: nil}, {x: 5, y: 2}, {x: 6, y: nil}] }
          assert2 { record.time_chart_xy_list2(:white, false) == [{x: 0, y: nil}, {x: 1, y: nil}, {x: 2, y: -3}, {x: 3, y: nil}, {x: 4, y: -7}, {x: 5, y: nil}] }
        end
        it "累計のデータが取れる" do
          assert2 { record.time_chart_xy_list2(:black, true) == [{x: 0, y: nil}, {x: 1, y: 1}, {x: 2, y: nil}, {x: 3, y: 6}, {x: 4, y: nil}, {x: 5, y: 8}, {x: 6, y: nil}] }
        end
      end
      ...
      describe "時間切れ" do
        ...
        it "後手の手番で時間切れなので残り秒数が取得できる" do
          assert2 { record.memberships[1].leave_alone_seconds == 590 }
        end
        it "後手の最大考慮時間は100ではなく500になっている" do
          assert2 { record.memberships[1].think_max == 590 }
        end
        it "後手のチャートの最後にそれを追加してある" do
          assert2 { record.time_chart_xy_list2(:black, false) == [{x: 0, y: nil}, {x: 1, y: 1}, {x: 2, y: nil}, {x: 3, y: 5}, {x: 4, y: nil}, {x: 5, y: 2}, {x: 6, y: nil}] }
          assert2 { record.time_chart_xy_list2(:white, false) == [{x: 0, y: nil}, {x: 1, y: nil}, {x: 2, y: -3}, {x: 3, y: nil}, {x: 4, y: -7}, {x: 5, y: nil}, {x: 6, y: -590}, {x: 7, y: nil}] }
        end
      ...
      describe "0手目で終了" do
        ...
        it "データがないときは0" do
          assert2 { record.memberships[0].think_max == 0 }
          assert2 { record.memberships[1].think_max == 0 }
        end
      end
    end
    ...
    describe "相入玉タグ" do
      ...
      it "works" do
        # "相居飛車" タグを除去している
        assert2 { record.memberships[0].note_tag_list == ["入玉", "相入玉", "居飛車"] }
