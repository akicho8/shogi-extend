    ...
    RSpec.describe History, type: :model, swars_spec: true do
      describe "development" do
        it "取得できる" do
          ...
          is_asserted_by { result.all_keys.present? }
          is_asserted_by { result.all_keys.all? { |e| e.user_keys.include?("YamadaTaro") } }
        end
        it "game_idを1つづず拾って重複していないこと" do
          result = History.new(user_key: "kinakom0chi").fetch
          is_asserted_by { result.all_keys.uniq == result.all_keys }
        end
      end
      it "production" do
        result = History.new(user_key: "chrono_", remote_run: true).fetch
        is_asserted_by { result.all_keys.present? }
        is_asserted_by { result.all_keys.all? { |e| e.user_keys.include?("chrono_") } }
      end
    end
  end
end
