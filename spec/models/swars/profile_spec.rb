...
module Swars
  RSpec.describe Profile, type: :model, swars_spec: true do
    ...
    describe "垢BAN" do
      ...
      it "ban_crowl_count_lteq: 垢BANチェック指定回数以下" do
        user = User.create!
        Profile.ban_crowl_count_lteq(0) == [user.profile]
      end

      it "ban_crowled_at_lt: 垢BANチェックの前回が指定日時より過去" do
        user = User.create!
        assert2 { Profile.ban_crowled_at_lt(user.profile.ban_crowled_at).count == 0     } # ban_crowled_at < ban_crowled_at     なので 0
        assert2 { Profile.ban_crowled_at_lt(user.profile.ban_crowled_at + 1).count == 1 } # ban_crowled_at < ban_crowled_at + 1 なので 1
      end
    end
  end
end
...
# >>   垢BAN
# >>     ban_crowled_at を更新すると ban_crowl_count をインクリメントする
# >>     ban_crowl_count_lteq: 垢BANチェック指定回数以下
# >>     ban_crowled_at_lt: 垢BANチェックの前回が指定日時より過去 (FAILED - 2)
# >>
# >> Failures:
# >>
...
# >>
# >>   2) Swars::Profile 垢BAN ban_crowled_at_lt: 垢BANチェックの前回が指定日時より過去
# >>      Failure/Error: Unable to find - to read failed line
...
# >>     0.1756 seconds -:40
# >>   Swars::Profile 垢BAN ban_crowled_at_lt: 垢BANチェックの前回が指定日時より過去
# >>     0.1344 seconds -:64
# >>   Swars::Profile 垢BAN ban_crowled_at を更新すると ban_crowl_count をインクリメントする
# >>     0.08252 seconds -:52
# >>   Swars::Profile 垢BAN ban_crowl_count_lteq: 垢BANチェック指定回数以下
# >>     0.06082 seconds -:59
# >>
...
# >> rspec -:64 # Swars::Profile 垢BAN ban_crowled_at_lt: 垢BANチェックの前回が指定日時より過去
# >>
