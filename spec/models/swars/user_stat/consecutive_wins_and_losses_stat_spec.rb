...
module Swars
  RSpec.describe UserStat::ConsecutiveWinsAndLossesStat, type: :model, swars_spec: true do
    describe "連勝・連敗" do
      ...
      it "10連勝" do
        case1([:win] * 10)
        assert { @user.user_stat.medal_stat.consecutive_wins_and_losses_stat.ten_win? }
        assert { @user.user_stat.medal_stat.active?(:"10連勝") }
      end

      it "波が激しいマン" do
        case1([:win] * 5 + [:lose] * 5)
        assert { @user.user_stat.medal_stat.consecutive_wins_and_losses_stat.waves_strong? }
