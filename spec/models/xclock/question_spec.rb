...
# == Schema Information ==
#
# Question (xclock_questions as Xclock::Question)
#
# |---------------------+---------------------+-------------+---------------------+------+-------|
...
#
#- Remarks ----------------------------------------------------------------------
# [Warning: Need to add relation] Xclock::Question モデルに belongs_to :lineage を追加してください
# [Warning: Need to add relation] Xclock::Question モデルに belongs_to :source_about を追加してください
# [Warning: Need to add relation] Xclock::Question モデルに belongs_to :user を追加してください
#--------------------------------------------------------------------------------
...
module Xclock
  RSpec.describe Question, type: :model do
    ...
    describe "update_from_js 保存" do
      ...
      it "works" do
        # 1つ目を作る
        question = user1.xclock_questions.build
    ...
    describe "フォルダ" do
      ...
      it "移動方法1" do
        user1.xclock_trash_box.questions << question1
        assert { question1.folder.class == Xclock::TrashBox }
      end
      it "移動方法2(フォーム用)" do
    ...
    it "公開フォルダに移動させたタイミングで投稿通知" do
      ...
      Xclock::LobbyMessage.destroy_all

      assert { Xclock::LobbyMessage.count == 0 }

      question1.update_from_js(folder_key: "active")
      assert { Xclock::LobbyMessage.count == 1 }
...
# >> ."position sfen 4k4/9/4G4/9/9/9/9/9/9 b G2r2b2g4s4n4l1p 1 moves G*5b"
# >> "position sfen 4k4/9/4G4/9/9/9/9/9/9 b G2r2b2g4s4n4l1p 1 moves G*5b"
# >> ["/Users/ikeda/src/shogi_web/app/models/xclock/question/info_mod.rb:80", :to_kif, "position sfen 4k4/9/4G4/9/9/9/9/9/9 b G2r2b2g4s4n4l1p 1 moves G*5b"]
# >> ."position sfen 4k4/9/4G4/9/9/9/9/9/9 b G2r2b2g4s4n4l1p 1 moves G*5b"
# >> "position sfen 4k4/9/4G4/9/9/9/9/9/9 b G2r2b2g4s4n4l1p 1 moves G*5b"
# >> ."position sfen 4k4/9/4G4/9/9/9/9/9/9 b G2r2b2g4s4n4l1p 1 moves G*5b"
# >> "position sfen 4k4/9/4G4/9/9/9/9/9/9 b G2r2b2g4s4n4l1p 1 moves G*5b"
