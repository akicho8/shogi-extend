...
# == Schema Information ==
#
# Moves answer (xclock_moves_answers as Xclock::MovesAnswer)
#
# |-----------------+-----------------+-------------+-------------+------+-------|
...
module Xclock
  RSpec.describe Question, type: :model do
    ...
    def test1(lineage_key)
      question = user1.xclock_questions.new
    ...
    describe "特殊なバリデーション" do
      ...
      it "validate4_all_piece_exists" do
        ...
        moves_answer = question.moves_answers.create(moves_str: MATE_HAND)
        moves_answer.errors.full_messages # => ["駒の数が変です。正確には香が1つ少ないのと歩が1つ少ないです。玉方の持駒を限定している詰将棋は「玉方持駒限定の似非詰将棋」にしといてください"]
        assert { moves_answer.errors.present? }
      end

      it "validate5_all_piece_not_exists" do
        moves_answer = test1("玉方持駒限定の似非詰将棋").moves_answers.create(moves_str: MATE_HAND)
