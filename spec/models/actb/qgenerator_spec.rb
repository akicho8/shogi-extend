...
module Actb
  RSpec.describe Qgenerator, type: :model do
    ...
    it "出題調整" do
      ...
      a_questions = 2.times.collect { user1.actb_questions.create_mock1 } # A
      b_questions = 2.times.collect { user1.actb_questions.create_mock1 } # B

      # B だけ履歴作成
      b_questions.each do |e|
        user1.actb_histories.create!(question: e, ox_mark: OxMark.fetch("correct"))
        user1.actb_histories.create!(question: e, ox_mark: OxMark.fetch("correct"))
      end

      users = [user1, user2]
      Qgenerator.new.history_latest_questions.to_sql # => "SELECT `actb_histories`.* FROM `actb_histories` WHERE `actb_histories`.`ox_mark_id` = 1 GROUP BY `actb_histories`.`question_id` ORDER BY MAX(created_at) DESC LIMIT 100"
