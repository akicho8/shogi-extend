...
RSpec.describe "なんでも棋譜変換", type: :model do
  ...
  describe "KENTO URL" do
    ...
    describe "initpos なし" do
      let :body do
        record = FreeBattle.create!(kifu_body: "https://www.kento-shogi.com/?moves=7g7f.3c3d")
        record.to_cached_kifu(:kif)
      end
      it do
        assert { body.include?("平手")              }
        assert { body.include?("まで2手で後手の勝ち") }
      end
    end
  end
end
