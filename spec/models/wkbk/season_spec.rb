...
# == Schema Information ==
#
# Season (wkbk_seasons as Wkbk::Season)
#
# |------------+------------+-------------+-------------+------+-------|
...
module Wkbk
  RSpec.describe Season, type: :model do
    ...
    it "単に新しいレコードを作るだけでユーザーの新シーズンに切り替わる" do
      ...
      assert { user1.wkbk_latest_xrecord.season.generation == 1 }
      assert { Season.create!.generation                  == 2 }
      assert { user1.wkbk_latest_xrecord.season.generation == 2 }
    end

    # xit "レーティングを引き継ぐ" do
    #   assert { user1.rating == 1500 }
    #   user1.wkbk_main_xrecord.update!(rating: 1501)
    #   assert { user1.rating  == 1501 }
    #   Wkbk::Season.create!
    #   assert { user1.wkbk_latest_xrecord.rating == 1501 }
    # end
    ...
    it "レーティングを引き継がない" do
      ...
      assert { user1.rating  == 1501 }
      Wkbk::Season.create!
      assert { user1.wkbk_latest_xrecord.rating == 1500 }
    end
