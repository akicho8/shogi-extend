...
module Wbook
  RSpec.describe Battle, type: :model do
    ...
    it "final_keyをセットしたタイミングで終了時刻も設定" do
      battle1.update!(final: Wbook::Final.fetch(:f_success))
      assert { battle1.end_at }
    end

    it "続きのバトル作成" do
      new_battle = battle1.battle_chain_create
      assert { new_battle.kind_of?(Wbook::Battle) }
    ...
    describe "#judge_final_set" do
      def test(judge_key)
        users = 2.times.collect {
          user = User.create!
          user.wbook_main_xrecord.skill_add(100)
          user.wbook_main_xrecord.save!
          user
        }
        ...
        battle.reload.memberships.flat_map do |e|
          [
            ...
            e.user.skill.key,
            e.user.skill_point,
            e.user.wbook_main_xrecord.skill_last_diff,
          ]
        end
      end
