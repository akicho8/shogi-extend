...
RSpec.describe KiwiMailer, type: :mailer do
  ...
  it "成功したので添付ファイルがある" do
    ...
    assert2 { mail.attachments.first.mime_type == "application/mp4" }
    assert2 { mail.attachments.first.filename.match?(/mp4/) }
  end
  ...
  it "失敗したのでいろいろない" do
    ...
    lemon1.main_process
    mail = KiwiMailer.lemon_notify(lemon1)
    assert2 { mail.body.decoded.match?(/失敗: bomb/) }
    assert2 { mail.attachments == [] }
  end
end
