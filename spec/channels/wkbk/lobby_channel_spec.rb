...
RSpec.describe Wkbk::LobbyChannel, type: :channel do
  ...
  describe "#unsubscribe" do
    ...
    it "対戦待ちリストを送信する" do
      expect { unsubscribe }.to have_broadcasted_to("wkbk/lobby_channel")
    end
  end
  ...
  describe "#matching_search" do
    ...
    context "同レートのマッチング" do
      it "マッチング" do
        ...
        # (少くとも)2回ブロードキャスト
        expect { matching_search(user2) }.to have_broadcasted_to("wkbk/lobby_channel").exactly(2)
        # 1. matching_user_ids_hash を伝える(←これはなくてもよくね？)
        # 2. 作成した battle を伝える

        assert { Wkbk::Room.count == 1 }
      end
    end
    ...
    context "レートを考慮したマッチング" do
      def user_of(rating)
        User.create!.tap do |e|
          e.wkbk_main_xrecord.update!(rating: rating)
        end
      end
  ...
  def matching_search(user, params = {})
    user.wkbk_setting.update!(session_lock_token: SecureRandom.hex)
    subscription.matching_search(params.merge(session_lock_token: user.wkbk_setting.session_lock_token))
  end
end
