...
RSpec.describe Actf::LobbyChannel, type: :channel do
  ...
  describe "#unsubscribe" do
    ...
    it "対戦待ちリストを送信する" do
      expect { unsubscribe }.to have_broadcasted_to("actf/lobby_channel").with(matching_list: [])
    end
  end
  ...
  describe "#matching_start" do
    ...
    context "同レートのマッチング" do
      ...
      it "マッチング" do
        ...
        # (少くとも)2回ブロードキャスト
        expect { subscription.matching_start({}) }.to have_broadcasted_to("actf/lobby_channel").twice
        # 1. matching_list を伝える(←これはなくてもよくね？)
        # 2. 作成した room を伝える

        assert { Actf::Room.count == 1 }
      end
    end
    ...
    context "レートを考慮したマッチング" do
      def user_of(rating)
        Colosseum::User.create!.tap do |e|
          e.actf_profile.update!(rating: rating)
        end
      end
