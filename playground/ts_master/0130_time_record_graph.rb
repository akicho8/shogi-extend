...
scope = scope.where(rule_key: "rule_mate7_type1")
scope.group("entry_name").order("count_all desc").having("count_all >= 10").count # => {}

names = scope.group("entry_name").pluck("entry_name") # => []
result = TimeRecord.select("entry_name, date(created_at) as created_on, min(spent_sec) as spent_sec").where(rule_key: "rule_mate7_type1").group("entry_name, date(created_at)")
