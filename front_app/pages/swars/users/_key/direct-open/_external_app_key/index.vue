  ...
  fetch() {
    ...
    if (this.external_app_setup) {
      this.$buefy.dialog.alert({
        ...
        message: `
          <div class="content">
            <p>直前の対局を<b>${this.external_app_info.name}</b>で毎回検討する人向けのショートカットの設定です</p>
            <p>この画面を<b>ホーム画面に追加</b> (またはブークマークに追加) すると、そこから直前の対局を最短で開けるようになります</p>
            <p>ウォーズ検索でIDを入力して検索して一番上の対局の外部アプリをタップする流れを自動化するイメージです</p>
          </div>`,
        confirmText: "わかった",
        type: 'is-info',
