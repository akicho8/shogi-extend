...
* URLパラメータ
  ...
  | RETRY_CONFIRM_DELAY      | SFEN送信後N秒待ってから返答をチェックする  |                                                                                         |
...
** 再送モーダル発動確認するには？ (待ち時間が短かすぎて発動)
   ...
   http://0.0.0.0:4000/share-board?room_code=a&force_user_name=alice&ordered_member_names=alice&RETRY_CONFIRM_DELAY=0&SEND_SUCCESS_DELAY=3
