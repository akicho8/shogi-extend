...
.WkbkQuestionEditApp
  ...
  .MainContainer(v-if="!$fetchState.pending")
    WkbkQuestionEditNavbar(:base="base")
    .container
    ...
    MainSection.is_mobile_padding_zero
      .container
        keep-alive
          WkbkQuestionEditPlacement(:base="base"  v-if="current_tab_info.key === 'placement_mode'")
          WkbkQuestionEditAnswerCreate(:base="base" v-if="current_tab_info.key === 'answer_create_mode'" ref="WkbkQuestionEditAnswerCreate")
          WkbkQuestionEditForm(:base="base"   v-if="current_tab_info.key === 'form_mode'")
          WkbkQuestionEditValidation(:base="base" v-if="current_tab_info.key === 'validation_mode'")
</template>
...
export default {
  ...
  fetch() {
    return this.$axios.$get("/api/wkbk.json", {params: {remote_action: "question_edit_fetch", ...this.$route.params, ...this.$route.query}}).then(e => {
  ...
  methods: {
    ...
    // FIXME: イベントで受けとる
    current_moves() {
      return this.$refs.WkbkQuestionEditAnswerCreate.$refs.main_sp.sp_object().moves_take_turn_offset
    },
    ...
    question_save_handle() {
      ...
      this.api_put("question_save_handle", {question: this.question}, e => {
        ...
        if (e.question) {
          ...
          this.$router.push({name: "wkbk-questions"})
        }
      })
    },
