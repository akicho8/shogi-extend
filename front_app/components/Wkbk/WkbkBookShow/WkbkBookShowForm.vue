  ...
  .columns.is-gapless
    .column
      ...
      b-field(label="ヒント" label-position="on-border" v-if="base.config.hint_enable")
        b-input(v-model.trim="base.book.hint_desc")

      //- b-field.lineage_key(label="種類" label-position="on-border" v-if="base.LineageInfo")
      //-   b-select(v-model="base.book.lineage_key" expanded)
      ...
      b-field(label="制限時間" label-position="on-border" v-if="base.config.time_limit_sec_enable")
        b-timepicker(v-model="base.book.time_limit_clock" icon="clock" :enable-seconds="true" :mobile-native="false")
      ...
      //-   b-switch(v-model="base.book.mate_skip" size="is-small") 最後は無駄合い (なので詰みチェックしない)
      ...
      //-   .box.py-5
      //-     b-field
      //-       b-switch(v-model="base.book.source_about_key" size="is-small" true-value="unknown" false-value="ascertained") 作者不詳
      //-
      //-     b-field(label="作者" label-position="on-border")
      //-       b-input(v-model.trim="base.book.source_author" placeholder="初代大橋宗桂" :disabled="base.book.source_about_key === 'unknown'")
      //-
      //-     b-field(label="出典メディア" label-position="on-border")
      //-       b-input(v-model.trim="base.book.source_media_name" placeholder="詰パラ")
      //-
      //-     b-field(label="出典年月日" label-position="on-border")
      //-       b-datepicker(
      //-         v-model="base.book.date_casted_source_published_on"
...
export default {
  ...
  created() {
    this.source_author_collapse_open_p = this.base.book.source_author_collapse_open_p
  },
  watch: {
    // "book.lineage_key": {
    //   handler(v) {
