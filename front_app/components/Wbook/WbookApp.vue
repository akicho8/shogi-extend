<template lang="pug">
.WbookApp(:class="mode")
  WbookEmotionApp( :base="base" v-if="mode === 'emotion'")
  WbookLobby(      :base="base" v-if="mode === 'lobby'")
  WbookRuleSelect( :base="base" v-if="mode === 'rule_select'")
  WbookMatching(   :base="base" v-if="mode === 'matching'")
  WbookBattleApp(  :base="base" v-if="mode === 'battle'")
  WbookResultApp(  :base="base" v-if="mode === 'result'")
  WbookRanking(    :base="base" v-if="mode === 'ranking'")
  WbookHistoryApp( :base="base" v-if="mode === 'history'")
  WbookBuilderApp( :base="base" v-if="mode === 'builder'" ref="builder")
  WbookMenuApp(    :base="base" v-if="mode === 'menu'")
  WbookChessClock( :base="base" v-if="mode === 'chess_clock'")
...
export default {
  ...
  data() {
    return {
      ...
      // リアクティブではないもの
      // $ac_school: null, // --> base/channels/wbook/school_channel.rb
      // $ac_lobby:  null, // --> base/channels/wbook/lobby_channel.rb
      // $ac_room:   null, // --> base/channels/wbook/room_channel.rb
      // $ac_battle: null, // --> base/channels/wbook/battle_channel.rb
    }
  },
  ...
  methods: {
    ...
    school_setup() {
      ...
      this.$ac_school = this.ac_subscription_create({channel: "Wbook::SchoolChannel"})
    },
    school_unsubscribe() {
    ...
    active_users_status_broadcasted(params) {
      if (params.wbook_school_user_ids) {
        this.school_user_ids = params.wbook_school_user_ids
      }
      if (params.wbook_room_user_ids) {
        this.room_user_ids = params.wbook_room_user_ids
      }
    },
    ...
    lobby_setup() {
      ...
      this.$ac_lobby = this.ac_subscription_create({channel: "Wbook::LobbyChannel"})
    ...
    menu_handle() {
      if (this.mode === "menu") {
        if (this.menu_component === "WbookMenuRoot") {
        } else {
          this.menu_to("WbookMenuRoot")
        }
      } else {
        this.lobby_unsubscribe()
        this.mode = "menu"
