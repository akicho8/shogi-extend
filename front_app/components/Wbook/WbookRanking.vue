...
.WbookRanking
  ...
  .WbookRankingRows(v-if="rank_data")
    WbookRankingRow(:base="base" v-for="row in rank_data.rank_records" :row="row")

    template(v-if="!rank_data.user_rank_in && rank_data.current_user_rank_record")
      WbookRankingRow.current_user_rank_record(:base="base" :row="rank_data.current_user_rank_record")
</template>
