...
.WbookHistoryApp
  ...
  template(v-if="base.history_current_tab_info.key === 'history_index'")
    WbookHistoryRow(:base="base" v-for="row in base.history_records" :row="row")

  template(v-if="base.history_current_tab_info.key === 'clip_index'")
    WbookHistoryRow(:base="base" v-for="row in base.clip_records" :row="row")
</template>
