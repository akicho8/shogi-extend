<template lang="pug">
.WbookApp.WbookUserShow.modal-card
  .modal-card-body.box
    ...
    .user_container.is-flex
      ...
      .skill_key.has-text-weight-bold.is-size-6.mt-1
        span.has-text-danger
          | {{ov_user_info.wbook_main_xrecord.skill_key}}
        span.has-text-danger.ml-1(v-if="base.config.rating_display_p")
          | {{rating_format(ov_user_info.wbook_main_xrecord.rating)}}
      ...
      nav.level.is-mobile.level_nav.mt-3
        .level-item.has-text-centered(v-if="false")
          div
            p.heading 対戦回数
            p.title {{ov_user_info.wbook_main_xrecord.battle_count}}
        .level-item.has-text-centered
          div
            p.heading 連勝数
            p.title {{ov_user_info.wbook_main_xrecord.straight_win_count}}
        .level-item.has-text-centered
          div
            p.heading 最多連勝数
            p.title {{ov_user_info.wbook_main_xrecord.straight_win_max}}
        .level-item.has-text-centered
          div
            p.heading 最多連敗数
            p.title {{ov_user_info.wbook_main_xrecord.straight_lose_max}}
        .level-item.has-text-centered
          div
            p.heading 切断回数
            p.title {{ov_user_info.wbook_main_xrecord.disconnect_count}}
...
export default {
  ...
  computed: {
    win_lose_circle_params() {
      return {
        judge_counts: {
          win:  this.ov_user_info.wbook_main_xrecord.win_count,
          lose: this.ov_user_info.wbook_main_xrecord.lose_count,
        },
      }
    },
    twitter_url() {
