    ...
    .mt-4
      b-menu
        b-menu-list(label="Action")
          b-menu-item(tag="nuxt-link" :to="{name: 'swars-users-key', params: {key: base.config.current_swars_user_key}}" @click.native="sound_play('click')" label="プレイヤー情報" :disabled="menu_item_disabled")

        b-menu-list(label="表示形式")
          b-menu-item(@click.stop="base.display_key_set('table')")
            template(slot="label")
          ...
          b-menu-item(label="開戦" @click.stop="base.display_key_set('critical')" :class="{'has-text-weight-bold': base.display_key === 'critical'}")
          b-menu-item(label="中盤" @click.stop="base.display_key_set('outbreak')" :class="{'has-text-weight-bold': base.display_key === 'outbreak'}")
          b-menu-item(label="終局" @click.stop="base.display_key_set('last')"     :class="{'has-text-weight-bold': base.display_key === 'last'}")
        ...
        b-menu-list(label="表示オプション")
          b-menu-item(@click="sound_play('click')")
            ...
            template(v-for="per in base.config.per_page_list")
              b-menu-item(:label="`${per}`" @click.stop="base.per_change_handle(per)" :class="{'has-text-weight-bold': per === base.config.per}")
        ...
        b-menu-list(label="便利な使い方あれこれ")
          ...
          b-menu-item(label="ホーム画面に追加" @click="base.bookmark_desc" :disabled="menu_item_disabled")
          ...
          b-menu-item(:disabled="menu_item_disabled" @click="sound_play('click')")
            ...
            template(v-for="e in base.ExternalAppInfo.values")
              b-menu-item(@click="base.external_app_handle(e)" :label="e.name")
