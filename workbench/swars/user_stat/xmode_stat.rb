require "../setup"
_ { Swars::User["SugarHuuko"].user_stat.xmode_stat.to_chart }        # => "215.23 ms"
...
# >>   Swars::Membership Ids (17.9ms)  SELECT `swars_memberships`.`id` FROM `swars_memberships` INNER JOIN `swars_battles` ON `swars_battles`.`id` = `swars_memberships`.`battle_id` WHERE `swars_memberships`.`user_id` = 17413 ORDER BY `swars_battles`.`battled_at` DESC LIMIT 50
