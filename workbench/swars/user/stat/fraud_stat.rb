require "./setup"
_ { Swars::User["SugarHuuko"].stat.fraud_stat.to_chart } # => "156.70 ms"
s { Swars::User["SugarHuuko"].stat.fraud_stat.to_chart } # => nil
...
# >>   Swars::Membership Ids (14.6ms)  SELECT `swars_memberships`.`id` FROM `swars_memberships` INNER JOIN `swars_battles` ON `swars_battles`.`id` = `swars_memberships`.`battle_id` WHERE `swars_memberships`.`user_id` = 17413 ORDER BY `swars_battles`.`battled_at` DESC LIMIT 50
...
# >>   ↳ app/models/swars/stat/fraud_stat.rb:13:in `count'
