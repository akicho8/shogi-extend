...
Bushido::TacticInfo.all_elements.each do |e|
  record = GeneralBattleRecord.tagged_with(e.name, on: "#{e.tactic_info.key}_tags").order(:turn_max).take
  if record
...
# 上のをコピペ
# rsync -avz s:/var/www/shogi_web_production/current/__tactic_infos/ ~/src/bushido/experiment/
# >> hit: 高美濃囲い
# >> hit: 銀冠穴熊
# >> hit: 横歩取り
# >> hit: 遠山流
