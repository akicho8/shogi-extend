...
# >>    (0.4ms)  COMMIT
# >>   Fanta::BattleRoom Load (0.3ms)  SELECT  `battle_rooms`.* FROM `battle_rooms` ORDER BY `battle_rooms`.`updated_at` DESC LIMIT 50
# >>   Fanta::User Load (0.3ms)  SELECT  `users`.* FROM `users` WHERE `users`.`id` = 35 LIMIT 1
# >>   Fanta::User Load (0.6ms)  SELECT `users`.* FROM `users` INNER JOIN `memberships` ON `users`.`id` = `memberships`.`user_id` WHERE `memberships`.`battle_room_id` = 16 ORDER BY `memberships`.`position` ASC
# >>   Fanta::User Load (0.4ms)  SELECT `users`.* FROM `users` INNER JOIN `watch_memberships` ON `users`.`id` = `watch_memberships`.`user_id` WHERE `watch_memberships`.`battle_room_id` = 16
# >>   Fanta::User Load (0.5ms)  SELECT `users`.* FROM `users` INNER JOIN `watch_memberships` ON `users`.`id` = `watch_memberships`.`user_id` WHERE `watch_memberships`.`battle_room_id` = 16
