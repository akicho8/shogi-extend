module Actb
  class BattleBroadcastJob < ApplicationJob
    ...
    def perform(battle)
      battle_json = battle.as_json(only: [:id, :rule_key, :rensen_index], include: { room: {}, memberships: { only: [:id], include: {user: { only: [:id, :name], methods: [:avatar_path] }} } }, methods: [:best_questions])
