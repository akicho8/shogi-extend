  ...
  def header_detail(h)
    ...
    row.each do |k, v|
      ...
      when /の(囲い|戦型)$/
        ...
        row[k] = v.collect { |e| h.link_to(e, h.swars_search_path(e)) }.join(" ").html_safe
      when "棋戦詳細"
        row[k] = v.collect { |e| h.link_to(e, h.swars_search_path(e)) }.join(" ").html_safe
      when "場所"
        if md = v.match(/(.*)「(.*?)」/)
        ...
        row[k] = v.collect { |e| h.link_to(e, h.swars_search_path(e)) }.join(" ").html_safe
      when "掲載"
        row[k] = h.link_to(v, h.swars_search_path(v))
      # when "持ち時間"
      #   row[k] = h.link_to(v, h.swars_search_path(v))
      # when "手合割"
      #   row[k] = h.link_to(v, h.swars_search_path(v))
      when /.手\z/
        row[k] = v.collect { |e| h.link_to(e, h.swars_search_path(e.slice(/\w+/))) }.join(" ").html_safe
      # when "棋戦"
      #   row[k] = h.link_to(v, h.swars_search_path(v))
      when "戦型"
        row[k] = h.link_to(v, h.swars_search_path(v))
