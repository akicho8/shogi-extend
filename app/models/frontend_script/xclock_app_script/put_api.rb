module FrontendScript
  class XclockAppScript
    concern :PutApi do
      ...
      # curl -d _method=put -d _user_id=1 -d remote_action=session_lock_token_set_handle -d session_lock_token=xxx http://localhost:3000/script/xclock-app.json
      def session_lock_token_set_handle
        raise ArgumentError, params.inspect if params[:session_lock_token].blank?
        # if current_user.xclock_setting.session_lock_token
        #   return { error_message: "" }
        # end
        current_user.xclock_setting.update!(session_lock_token: params[:session_lock_token])
        { status: "success" }
      end

      # curl -d _method=put -d _user_id=1 -d remote_action=session_lock_token_valid_handle -d session_lock_token=xxx http://localhost:3000/script/xclock-app.json
      ...
      # curl -d _method=put -d _user_id=1 -d remote_action=session_lock_token_reset_handle http://localhost:3000/script/xclock-app.json
      # def session_lock_token_reset_handle
      #   current_user.xclock_setting.update!(session_lock_token: nil)
      #   { status: "success" }
      # end

      # curl -d _method=put -d _user_id=1 -d remote_action=rule_key_set_handle -d rule_key=sy_marathon http://localhost:3000/script/xclock-app.json
      def rule_key_set_handle
        ...
        current_user.xclock_setting.update!(rule: Xclock::Rule.fetch(params[:rule_key]))
        { status: "success" }
      end
      ...
      def debug_matching_add_handle
        if user = User.where.not(id: params[:exclude_user_id]).first
          Xclock::Rule.matching_users_delete_from_all_rules(user)
          if rule_key = params[:rule_key]
            rule = Xclock::Rule.fetch(rule_key)
            user.xclock_setting.update!(rule: rule)
          else
            rule = current_user.xclock_setting.rule
          end
      ...
      def matching_users_clear_handle
        Xclock::Rule.matching_users_clear
        { status: "success" }
      end
      ...
      def new_challenge_accept_handle
        ...
        rule = Xclock::Rule.fetch(params[:rule_key])
        ...
        if Xclock::BaseChannel.once_run(key, expires_in: 3.seconds)
          current_user.xclock_setting.update!(session_lock_token: params[:session_lock_token])
          Xclock::Room.create_with_members!([user, current_user], rule: rule)
      ...
      #   raise ArgumentError if params[:session_lock_token].blank?
      #
      #   ids = Xclock::Rule.matching_all_user_ids
      ...
      #   user = User.find(id)
      #   current_user.xclock_setting.update!(session_lock_token: params[:session_lock_token])
      #   Xclock::Room.create_with_members!([user, current_user], rule: user.xclock_setting.rule)
      #   { status: "success" }
      # end
      ...
      def question_save_handle
        if id = params[:question][:id]
          question = Xclock::Question.find(id)
        else
          question = current_user.xclock_questions.build
        end
        begin
      ...
      def emotion_save_handle
        ...
        { emotions: current_user.emotions.reload.as_json(Xclock::Emotion.json_type13) }
      end
      ...
      def emotions_import_handle
        Xclock::EmotionInfo.each do |e|
          folder = Xclock::EmotionFolder.fetch(e.folder_key)
          current_user.emotions.create(folder: folder, name: e.name, message: e.message, voice: e.voice)
        end
        { emotions: current_user.emotions.reload.as_json(Xclock::Emotion.json_type13) }
      end

      # curl -d _method=put -d user_name=a -d remote_action=user_profile_update -d _user_id=1 http://localhost:3000/script/xclock-app
      ...
      # from app/javascript/xclock_app/the_profile_edit_form.vue profile_update_handle
