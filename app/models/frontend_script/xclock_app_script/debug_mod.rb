  ...
  class XclockAppScript
    concern :DebugMod do
      ...
      def debug_for_result(info)
        ...
        users.each do |user|
          record = user.xclock_main_xrecord
          record.skill_key = "A+"
          record.skill_point = 50
          record.save!
        end

        record = users[0].xclock_main_xrecord
        record.skill_add(60)
        record.save!

        record = users[1].xclock_main_xrecord
        record.skill_add(60)
        record.save!

        room = Xclock::Room.create_with_members!(users)
        battle = room.battle_create_with_members!(final: Xclock::Final.fetch(:f_disconnect))
        battle.memberships[0].update!(judge: Xclock::Judge.fetch(:win))
        battle.memberships[1].update!(judge: Xclock::Judge.fetch(:lose))
        battle.reload
      ...
      def debug_for_ov_user_info(info)
        ...
        e = current_user.xclock_main_xrecord
        e.win_count = 1
        e.lose_count = 2
        e.save!
      ...
      def debug_for_battle_sy(info, rule_key)
        ...
        room = Xclock::Room.create_with_members!(users, rule: rule)
