...
#   app/models/acns3.rb
#   app/models/colosseum/user_acns3_mod.rb
#
#   question
#     app/models/acns3/question.rb
#     app/models/acns3/moves_answer.rb
#     app/models/acns3/endpos_answer.rb
#
# channel
#   app/channels/acns3/lobby_channel.rb
#   app/channels/acns3/room_channel.rb
#
# job
...
module FrontendScript
  class Acns3SampleScript < ::FrontendScript::Base
    ...
    # http://localhost:3000/admin/script/acns3-sample.json?index_fetch=true
    def script_body
      if params[:index_fetch]
        params[:per] ||= PER_DEFAULT

        s = current_user.acns3_questions
      ...
      # }.deep_symbolize_keys
      #
      # question = current_user.acns3_questions.find_or_initialize_by(id: params[:question][:id])
      # question.together_with_params_came_from_js_update(params)
      # return question.create_the_parameters_to_be_passed_to_the_js

      # question = current_user.acns3_questions.create! do |e|
      ...
      # if !current_room
      #   out += Acns3::Room.order(:id).collect { |room|
      #     {
    ...
    def debug_scene_set(info)
      ...
      if current_debug_scene == :result_show
        ...
        room = Acns3::Room.create!(final_key: :disconnect) do |e|
          e.memberships.build(user: user1, judge_key: :win,  quest_index: 1)
          e.memberships.build(user: user2, judge_key: :lose, quest_index: 2)
        end

        info[:room] = room.as_json(only: [:id], include: { memberships: { only: [:id, :judge_key, :rensho_count, :renpai_count, :quest_index], include: {user: { only: [:id, :name], methods: [:avatar_path], include: {acns3_profile: { only: [:id, :rensho_count, :renpai_count, :rating, :rating_max, :rating_last_diff, :rensho_max, :renpai_max] } } }} }}, methods: [:simple_quest_infos, :final_info])
      end
