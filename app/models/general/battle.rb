...
class General::Battle < ApplicationRecord
  ...
  def gstate_info
    General::GstateInfo.fetch(battle_state_key)
  end
  ...
  concerning :ConvertHookMethos do
    class_methods do
      ...
      def import(key, **params)
        begin
          p [Time.current.to_s(:ymdhms), "begin", General::User.count, General::Battle.count] unless Rails.env.test?
          ...
          unless Rails.env.test?
            puts
            p [Time.current.to_s(:ymdhms), "end__", General::User.count, General::Battle.count, error].compact
          end
        end
      end

      # rails r 'General::Battle.destroy_all; General::Battle.all_import'
      # capp rails:runner CODE='General::Battle.all_import'
      # rails r 'General::Battle.all_import(kifu_dir: "..")'
    ...
    def parser_exec_after(info)
      ...
      meta_info[:simple_names].each do |pair|
        pair.each do |names|
          General::User.find_or_create_by(name: names.first)
        end
      end
