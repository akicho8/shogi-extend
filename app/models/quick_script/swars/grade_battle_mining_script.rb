...
# 一次集計
# rails r QuickScript::Swars::GradeBattleMiningScript.new.cache_write
# rails r 'QuickScript::Swars::GradeBattleMiningScript.new({}, {need_size: 200}).cache_write'
#
# http://localhost:4000/lab/swars/tactic-cross2
#
module QuickScript
  module Swars
    class GradeBattleMiningScript < Base
      ...
      def human_rows
        rows = aggregate.sort_by { |key, _| ::Swars::GradeInfo[key].priority }
        rows.collect do |key, battle_ids|
          {
            "棋力" => item_name_search_link(key),
            "件数" => battle_ids.size,
          }
        end
      end
      ...
      concerning :AggregateMethods do
        ...
        def battle_ids_of(grade_info)
          ...
          [
            ...
            ids = finder(ids, grade_info, e)
          end
          ids
        end

        def finder(ids, grade_info, condition_method)
          if ids.size < need_size
            main_scope.in_batches(order: :desc, of: batch_size).each.with_index do |scope, batch_index|
              scope = scope.joins(:grade)
              scope = scope.where(::Swars::Grade.arel_table[:key].eq(grade_info.key))
        ...
        def grade_infos
          @grade_infos ||= yield_self do
            av = ::Swars::GradeInfo.active_only
            if v = @options[:grade_keys]
