...
- 再対局
  - 「初期配置に戻す」をしたあとルール設定を行う (以下同様)
  - 部屋を作り直す必要はない

##### 推奨ブラウザは？

- PC でもスマホでも Google Chrome 一択
- バグだらけの Safari にはもう対応するのを諦めた
...
- 「入室」の中にある「部屋のリンク」またはメニュー内の「部屋のリンクのコピー」をタップする
...
- ☗または☖の大きい方
- だったが現在は☗☖の位置にアバターを表示しているのでわからない
...
- ルール設定で「反則」を「待ったできる」にしておく
- そして待ったモーダルが出た瞬間、自分で「待った」を押す
...
- 一応ある
- しかし次の操作をしたとき情報をリセットするため正確でない場合がある
  - 待った等で手数(局面)を動かす
  - 指し手以外で特定の局面を受信する
  - 部屋に入り直す
...
- 本譜の棋譜を持っていて「ルール設定がOFF」かつ「時計がOFFまたはPAUSE」のとき
...
- 「ハンドルネーム変更」
- 入室後の場合は「入室」から退室して新しいハンドルネームを入力して入室する
...
- 「手番を知らせる効果音」と「次は○○の手番です」の発声はルール設定の面子が3人以上のときだけ
- 2人以下では順番が明確なので鳴らない
...
- ルール設定している状況では「先手」「後手」「観戦」の項目に名前が入る
- ルール設定していない状況では「面子」にすべてのメンバーの名前が入る
