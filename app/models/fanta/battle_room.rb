...
class Fanta::BattleRoom < ApplicationRecord
  time_rangable default: false

  has_many :current_users, class_name: "Fanta::User", foreign_key: :current_battle_room_id, dependent: :nullify
  ...
  def name
    ...
    #   end
    #   names << "対戦部屋 ##{Fanta::BattleRoom.count.next}"
    #   names.join
    # end
    "##{id}"
  end
  ...
  def xstate_info
    Fanta::XstateInfo.fetch(xstate_key)
  end
