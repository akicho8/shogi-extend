    ...
    concern :VoteMod do
      included do
        with_options dependent: :destroy do
          has_many :wbook_good_marks, class_name: "Wbook::GoodMark"
          has_many :wbook_bad_marks,  class_name: "Wbook::BadMark"
        end
      end
      ...
      def good_p(question)
        wbook_good_marks.where(question: question).exists?
      end

      def bad_p(question)
        wbook_bad_marks.where(question: question).exists?
      end
      ...
      def vote_scope(question, vote)
        send("wbook_#{vote.key}_marks").where(question: question)
      end
