module Wbook::Question::ImportExportMod
  ...
  class_methods do
    ...
    def export_all
      ...
      file = Rails.root.join("app/models/wbook/#{name.demodulize.underscore.pluralize}.yml")
    ...
    def import_all(user = User.sysop)
      persistent_records.each do |e|
        record = user.wbook_questions.find_or_initialize_by(key: e[:key])
    ...
    def persistent_file
      Rails.root.join("app/models/wbook/#{name.demodulize.underscore.pluralize}.yml")
    end
