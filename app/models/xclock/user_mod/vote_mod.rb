    ...
    concern :VoteMod do
      included do
        with_options dependent: :destroy do
          has_many :xclock_good_marks, class_name: "Xclock::GoodMark"
          has_many :xclock_bad_marks,  class_name: "Xclock::BadMark"
        end
      end
      ...
      def good_p(question)
        xclock_good_marks.where(question: question).exists?
      end

      def bad_p(question)
        xclock_bad_marks.where(question: question).exists?
      end
      ...
      def vote_scope(question, vote)
        send("xclock_#{vote.key}_marks").where(question: question)
      end
