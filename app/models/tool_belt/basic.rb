    ...
    def build
      ...
      out << h.tag.div(:class => "buttons") do
        [
          ...
          link_to_eval("lovelive 取り込み", redirect_to: [:swars, :battles]) { "Swars::Importer::AllRuleImporter.new(user_key: 'lovelive', remote_run: true).run"      },
          link_to_eval("itoshinTV 取り込み", redirect_to: [:swars, :battles]) { "Swars::Importer::AllRuleImporter.new(user_key: 'itoshinTV', remote_run: true).run"      },
          link_to_eval("misaka_level5 取り込み", redirect_to: [:swars, :battles]) { "Swars::Importer::AllRuleImporter.new(user_key: 'misaka_level5', remote_run: true).run"      },
          link_to_eval("aika0603 取り込み", redirect_to: [:swars, :battles]) { "Swars::Importer::AllRuleImporter.new(user_key: 'aika0603', remote_run: true).run"      },
          link_to_eval("YamadaTaro 取り込み", redirect_to: [:swars, :battles]) { "Swars::Importer::AllRuleImporter.new(user_key: 'YamadaTaro', remote_run: true).run"      },
