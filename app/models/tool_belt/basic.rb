    ...
    def build
      ...
      out << h.tag.div(:class => "buttons") do
        [
          ...
          link_to_eval("lovelive 取り込み", redirect_to: [:swars, :battles]) { "Swars::Importer::UserImporter.new(user_key: 'lovelive', run_remote: true).run"      },
          link_to_eval("itoshinTV 取り込み", redirect_to: [:swars, :battles]) { "Swars::Importer::UserImporter.new(user_key: 'itoshinTV', run_remote: true).run"      },
          link_to_eval("misaka_level5 取り込み", redirect_to: [:swars, :battles]) { "Swars::Importer::UserImporter.new(user_key: 'misaka_level5', run_remote: true).run"      },
          link_to_eval("aika0603 取り込み", redirect_to: [:swars, :battles]) { "Swars::Importer::UserImporter.new(user_key: 'aika0603', run_remote: true).run"      },
          link_to_eval("chrono_ 取り込み", redirect_to: [:swars, :battles]) { "Swars::Importer::UserImporter.new(user_key: 'chrono_', run_remote: true).run"      },
          link_to_eval("Ishitaka_Sumie 取り込み (指導対局)")                  { "Swars::Importer::UserImporter.new(user_key: 'Ishitaka_Sumie', run_remote: true).run" },
          link_to_eval("kinakom0chi 取り込み")                                { "Swars::Importer::UserImporter.new(user_key: 'kinakom0chi', run_remote: true).run"    },
