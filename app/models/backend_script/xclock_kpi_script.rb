module BackendScript
  class XclockKpiScript < ::BackendScript::Base
    include SortMod
    ...
    def script_body
      ...
      history_hash = {}
      if true
        model = Xclock::History
        s = model.all
        s = s.where(model.arel_table[:created_at].gteq(time_begin))
        records = s.select([
            "DATE(#{DbCop.tz_adjust(:created_at)}) AS created_on",                          # 時間→日付変換
            "COUNT(*) AS count_all",                                                        # 履歴数
            "COUNT(ox_mark_id = #{Xclock::OxMark.fetch(:correct).id} or NULL) AS correct_count", # o
            "COUNT(ox_mark_id = #{Xclock::OxMark.fetch(:mistake).id} or NULL) AS mistake_count", # x
            "COUNT(ox_mark_id = #{Xclock::OxMark.fetch(:timeout).id} or NULL) AS timeout_count", # -
          ].join(", ")).group("created_on")                                  # 日付毎
        # 日付から一発で対応するレコードを求められるようにハッシュ化
