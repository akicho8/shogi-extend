    ...
    def script_body
      ...
      if true
        ...
        records = s.select([
            ...
            "COUNT(ox_mark_id = #{Wkbk::OxMark.fetch(:correct).id} or NULL) AS correct_count", # o
            "COUNT(ox_mark_id = #{Wkbk::OxMark.fetch(:mistake).id} or NULL) AS mistake_count", # x
            "COUNT(ox_mark_id = #{Wkbk::OxMark.fetch(:timeout).id} or NULL) AS timeout_count", # -
          ].join(", ")).group("created_on")                                  # 日付毎
        # 日付から一発で対応するレコードを求められるようにハッシュ化
