module BackendScript
  class XmovieDashboardScript < ::BackendScript::Base
    include SortMethods

    self.category = "xmovie"
    ...
    def script_body
      ...
      if true
        ...
        xmovie_record_hash = records.inject({}) { |a, e| a.merge(e[:created_on] => e) }
      end
      ...
      current_length.times.collect do |i|
        ...
        row["変換数"]   = xmovie_record_hash[date]&.count_all
        row["変換人数"] = xmovie_record_hash[date]&.unique_user_id_count
        row["失敗"]     = xmovie_record_hash[date]&.errored_count
        row
      end
    end
