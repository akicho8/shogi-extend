module XyMaster
  class ScopeInfo
    ...
    memory_record [
      { key: "scope_today",      name: "本日", key_method: :table_key_for_today,      date_visible: false, table_key_for: -> time_record, rule_info { rule_info.ymd_table_key_for_time(time_record.created_at) }, time_record_scope: -> scope { scope.where(created_at: Time.current.midnight...Time.current.midnight.tomorrow)                       }},
      { key: "scope_yesterday",  name: "昨日", key_method: :table_key_for_yesterday,  date_visible: false, table_key_for: -> time_record, rule_info { rule_info.ymd_table_key_for_time(time_record.created_at) }, time_record_scope: -> scope { scope.where(created_at: Time.current.midnight.yesterday...Time.current.midnight)                      }},
