  ...
  class Battle
    concern :CleanerMethods do
      included do
        ...
        scope :analysis_version_old_only, -> { where("analysis_version <  ?", Bioshogi::ANALYSIS_VERSION) }           # 戦法抽出バージョンが古いもの
        scope :analysis_version_new_only, -> { where("analysis_version >= ?", Bioshogi::ANALYSIS_VERSION) }           # 戦法抽出バージョンが新しいもの
        ...
        scope :destroyable_n, -> (options = {}) {
          options = {
            ...
            :ban_except  => false,
            :old_only    => Rails.env.local? ? 0.days : 1.days,
