...
class Swars::Battle < ApplicationRecord
  ...
  before_validation on: :create do
    ...
    # "" から ten_min への変換
    if rule_key
      self.rule_key = Swars::RuleInfo.fetch(rule_key).key
    end
  ...
  concerning :ImportMethods do
    class_methods do
      ...
      def import(key, **params, &block)
        counts = -> { Vector[Swars::User.count, Swars::Battle.count] }
        old = counts.call
        begin
      ...
      # Swars::Battle.reception_import(limit: 10, sleep: 5)
      ...
      # Swars::Battle.conditional_import(limit: 10, page_max: 3, sleep: 5, grade_key_gteq: "初段") # (10 * (3*10) * 5) / 60 = 25 min
      def conditional_import(**params)
        ...
        if true
          if v = params[:grade_key_gteq]
            priority = Swars::GradeInfo.fetch(v).priority
            s = s.joins(user: :grade).where(Swars::Grade.arel_table[:priority].lteq(priority))
          end
        end
        s = s.group(:user_id).select(:user_id)
        s = s.joins(:battle).order("max(#{Swars::Battle.table_name}.battled_at) desc")
        ...
        # user_ids = Swars::Membership.group(:user_id).select(:user_id).order("max(created_at) desc").limit(params[:limit] || 1).pluck(:user_id)
      ...
      # Swars::Battle.basic_import(user_key: "DarkPonamin9")
      # Swars::Battle.basic_import(user_key: "micro77")
      # Swars::Battle.basic_import(user_key: "micro77", page_max: 3)
      def basic_import(**params)
        Swars::RuleInfo.each do |e|
      ...
      def multiple_battle_import(**params)
        (params[:page_max] || 1).times do |i|
          ...
          list.each do |history|
            ...
            # すでに取り込んでいるならスキップ
            if Swars::Battle.where(battle_key: battle_key).exists?
              next
            end
            ...
            #   # 初段以上の指定がある場合
            #   if v = params[:grade_key_gteq]
            #     v = Swars::GradeInfo.fetch(v)
            #     # 取得してないときもあるため
            #     if user_infos = history[:user_infos]
            #       # 両方初段以上ならOK
            #       if user_infos.all? { |e| Swars::GradeInfo.fetch(e[:grade_key]).priority <= v.priority }
            #       else
            #         next
            #       end
            #     end
      ...
      def single_battle_import(battle_key)
        ...
        # SQLをシンプルにするために勝者だけ、所有者的な意味で、Swars::Battle 自体に入れとく
        # いらんかったらあとでとる
        if winner_index
