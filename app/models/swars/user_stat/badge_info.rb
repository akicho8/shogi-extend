  ...
  module UserStat
    class BadgeInfo
      ...
      memory_record [
        ...
        { key: "無気力マン",         badge_params: { name: "🦥",   message: "無気力な対局をした",                      }, if_cond: proc { user_stat.lethargy_stat.exist? } },
        { key: "大長考マン",         badge_params: { name: "😴",   message: "対局放棄と受け取られかねない長考をした",  }, if_cond: proc { user_stat.prolonged_deliberation_stat.count.positive? } },
        { key: "1手詰じらしマン",    badge_params: { name: "😈",   message: "1手詰を焦らして歪んだ優越感に浸った",     }, if_cond: proc { user_stat.mate_stat.count.positive? } },
        { key: "相手退席待ちマン",   badge_params: { name: "🪰",   message: AITETAISEKIMATMAN_MESSAGE,                 }, if_cond: proc { user_stat.waiting_to_leave_stat.count.positive? } },
        ...
        { key: "急戦マン",           badge_params: { name: "🐝",    message: "急戦で勝ち越した",                   }, if_cond: proc { user_stat.rapid_attack_stat.badge? },},
        ...
        { key: "200手越えマン",      badge_params: { name: "⚡️",    message: "200手以上で勝った",                  }, if_cond: proc { (user_stat.win_turn_stat.max || 0) >= 200 },},
        { key: "心強すぎマン",       badge_params: { name: "🫀",   message: "折れない心の持ち主",                 }, if_cond: proc { user_stat.mental_stat.hard_brain? },},
        { key: "廃指しマン",         badge_params: { name: "😡",   message: "廃指し名人",                         }, if_cond: proc { (user_stat.daily_average_matches_stat.max || 0) >= 30 },},

        { key: "投了マン",           badge_params: { name: "🙇‍♂️", message: "負けるときは必ず投了する",            }, if_cond: proc { (user_stat.judge_final_stat.ratio_by(:lose, :TORYO) || 0) >= 1.0 }, },
        { key: "切れ負けマン",       badge_params: { name: "⌛",   message: "切れ負けが多い",                      }, if_cond: proc { (user_stat.judge_final_stat.ratio_by(:lose, :TIMEOUT) || 0) >= 0.25 },},
        { key: "レア戦法マン",       badge_params: { name: "🍀",   message: "変態戦法の使い手",                   }, if_cond: proc { user_stat.rarity_stat.minority?                    },},
        { key: "長考マン",           badge_params: { name: "🤯",   message: "考えすぎて負けがち",                 }, if_cond: proc { user_stat.overthinking_loss_stat.badge? } },
        { key: "開幕千日手",         badge_params: { name: "❓",   message: "開幕千日手をした",                   }, if_cond: proc { (user_stat.perpetual_check_stat.opening_repetition_move_count || 0).positive? } },
        { key: "ただの千日手",       badge_params: { name: "🍌",   message: "千日手の使い手",                     }, if_cond: proc { (user_stat.perpetual_check_stat.over50_draw_count || 0).positive? } },
        { key: "友対勝ちマン",       badge_params: { name: "🆚",   message: "友達対局で勝った",                   }, if_cond: proc { user_stat.xmode_judge_stat.exist?(:"友達", :win) } },
        { key: "指導受けマン",       badge_params: { name: "🔥",   message: "指導対局で負けた",                   }, if_cond: proc { user_stat.xmode_judge_stat.exist?(:"指導", :lose) } },
        { key: "プロ越えマン",       badge_params: { name: "💪",   message: "指導対局で勝った",                   }, if_cond: proc { user_stat.xmode_judge_stat.exist?(:"指導", :win) } },
        { key: "運営支えマン",       badge_params: { name: "🧙‍♂️", message: "将棋ウォーズの運営を支える力がある", }, if_cond: proc { user_stat.fraud_stat.count.positive? } },
      ]
    end
  end
end
