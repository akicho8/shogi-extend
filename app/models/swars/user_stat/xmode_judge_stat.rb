...
module Swars
  module UserStat
    class XmodeJudgeStat < Base
      ...
      # Swars::User["SugarHuuko"].user_stat.xmode_judge_stat.exist?(:"野良", :win)
      def exist?(xmode_key, judge_key)
        key = [xmode_key.to_s, judge_key.to_s]
        counts_hash.has_key?(key)
      end

      # Swars::User["SugarHuuko"].user_stat.xmode_judge_stat.count_by(:"野良", :win) # => 39
      def count_by(xmode_key, judge_key)
        key = [xmode_key.to_s, judge_key.to_s]
        counts_hash.fetch(key, 0)
      end

      # Swars::User["SugarHuuko"].user_stat.xmode_judge_stat.ratio_by(:"野良", :win) # => 0.78
