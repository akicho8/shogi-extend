module Emox
  module UserMod
    ...
    included do
      ...
      # このユーザーが作成した問題(複数)
      has_many :emox_questions, class_name: "Emox::Question", dependent: :destroy do
      ...
      # このユーザーに出題した問題(複数)
      has_many :emox_histories, class_name: "Emox::History", dependent: :destroy

      # 自分がBOTになった部屋
      has_many :emox_bot_rooms, class_name: "Emox::Room", foreign_key: :bot_user_id, dependent: :destroy

      # 通知
      has_many :notifications, class_name: "Emox::Notification", dependent: :destroy # 自分が受信
    end

    concerning :OtherMethods do
      def page_url(options = {})
        UrlProxy.wrap2("/emox?user_id=#{id}")
        # Rails.application.routes.url_helpers.url_for([:emox, {only_path: false, user_id: id}.merge(options)])
      end
    ...
    concerning :OUcountNotifyMod do
      ...
      def straight_win_count_notify
        User.bot.lobby_speak("#{linked_name}さんが#{emox_season_xrecord.straight_win_count}連勝しました")
      end
    ...
    concerning :CurrentUserMethods do
      ...
      def as_json_type9
        as_json({
            ...
            include: {
              emotions: Emox::Emotion.json_type13,
            },
            methods: [
              :avatar_path,
              :rating,
      ...
      # レギュラー条件
      def regular_p
        emox_room_memberships.count >= 1 || emox_questions.active_only.count >= 1
      end
    ...
    concerning :MessageMethods do
      included do
        with_options dependent: :destroy do
          has_many :emox_room_messages,     class_name: "Emox::RoomMessage"
          has_many :emox_lobby_messages,    class_name: "Emox::LobbyMessage"
          has_many :emox_question_messages, class_name: "Emox::QuestionMessage"
        end
      end

      # rails r 'User.sysop.lobby_speak(Time.current)'
      def lobby_speak(message_body, options = {})
        emox_lobby_messages.create!({body: message_body}.merge(options))
      end

      # rails r 'User.sysop.room_speak(Emox::Room.first, Time.current)'
      def room_speak(room, message_body, options = {})
        emox_room_messages.create!({room: room, body: message_body}.merge(options))
      end

      # rails r 'User.sysop.question_speak(Emox::Question.first, Time.current)'
      def question_speak(question, message_body, options = {})
        emox_question_messages.create!({question: question, body: message_body}.merge(options))
      end
    end

    concerning :MainXrecordMod do
      included do
        has_one :emox_main_xrecord, class_name: "Emox::MainXrecord", dependent: :destroy

        delegate :rating, :straight_win_count, :straight_win_max, :skill, :skill_key, :skill_point, to: :emox_main_xrecord

        after_create do
          create_emox_main_xrecord!
        end
      end

      def create_emox_main_xrecord_if_blank
        emox_main_xrecord || create_emox_main_xrecord!
      end

      # 両方に同じ処理を行う
      def both_xrecord_each
        [:emox_main_xrecord, :emox_latest_xrecord].each do |e|
          yield public_send(e)
        end
      end
    end
    ...
    concerning :SeasonXrecordMod do
      included do
        # プロフィール
        with_options class_name: "Emox::SeasonXrecord", dependent: :destroy do
          has_one :emox_season_xrecord, -> { newest_order }
          has_many :emox_season_xrecords
        end

        after_create do
          create_emox_season_xrecord_if_blank
        end
      end

      def create_emox_season_xrecord_if_blank
        emox_latest_xrecord
      end
      ...
      # 必ず存在する最新シーズンのプロフィール
      def emox_latest_xrecord
        emox_season_xrecords.find_or_create_by!(season: Season.newest)
      end
    end
    ...
    concerning :EmotionMod do
      included do
        ...
        has_many :emotions, class_name: "Emox::Emotion", dependent: :destroy
      end
    end
    ...
    concerning :UserInfoMethods do
      ...
      def active_questions_count
        emox_questions.active_only.count
      end

      def questions_good_rate_average
        emox_questions.average(:good_rate)
      end

      def questions_good_marks_total
        emox_questions.sum(:good_marks_count)
      end

      def questions_bad_marks_total
        emox_questions.sum(:bad_marks_count)
      end

      def total_o_count
        emox_histories.with_o.count
      end

      def total_x_count
        emox_histories.with_ox_mark(:mistake).count
      end

      def today_total_o_count
        emox_histories.with_today.with_o.count
      end

      def today_total_x_count
        emox_histories.with_today.with_ox_mark(:mistake).count
      end

      def today_total_o_ucount
        emox_histories.with_today.with_o.distinct.count(:question_id)
      end

      def today_total_x_ucount
        emox_histories.with_today.with_ox_mark(:mistake).distinct.count(:question_id)
      end
      ...
      def info
        {
          ...
          "レーティング"       => rating,
          "クラス"             => skill_key,
          "最新シーズン情報ID" => emox_latest_xrecord.id,
          "永続的プロフ情報ID" => emox_main_xrecord.id,
          "部屋入室数"         => emox_room_memberships.count,
          "対局数"             => emox_battle_memberships.count,
          "問題履歴数"         => emox_histories.count,
          "バトル中発言数"     => emox_room_messages.count,
          "ロビー発言数"       => emox_lobby_messages.count,
          "問題コメント数"     => emox_question_messages.count,

          "作成問題数"         => emox_questions.count,
          "問題高評価率"       => emox_questions.average(:good_rate),
          "問題高評価数"       => emox_questions.sum(:good_marks_count),
          "問題低評価数"       => emox_questions.sum(:bad_marks_count),
