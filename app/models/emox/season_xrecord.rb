...
# == Schema Information ==
#
# Season xrecord (emox_season_xrecords as Emox::SeasonXrecord)
#
# |---------------------+---------------------+------------+-------------+--------------+-------|
...
module Emox
  class SeasonXrecord < ApplicationRecord
    ...
    before_validation do
      ...
      self.create_count ||= user.emox_season_xrecords.count.next
    end
    ...
    # 連勝通知
    # rails r "User.first.emox_latest_xrecord.judge_set(Judge.fetch(:win))"
