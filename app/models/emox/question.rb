...
# == Schema Information ==
#
# Question (emox_questions as Emox::Question)
#
# |---------------------+---------------------+-------------+---------------------+------+-------|
...
#
#- Remarks ----------------------------------------------------------------------
# [Warning: Need to add relation] Emox::Question モデルに belongs_to :lineage を追加してください
# [Warning: Need to add relation] Emox::Question モデルに belongs_to :source_about を追加してください
# [Warning: Need to add relation] Emox::Question モデルに belongs_to :user を追加してください
#--------------------------------------------------------------------------------
...
module Emox
  class Question < ApplicationRecord
    ...
    # rails r 'Emox::Question.tag_normalize_all; tp Emox::Question'
    ...
    has_many :messages, class_name: "Emox::QuestionMessage", dependent: :destroy # コメント
    ...
    def page_url(options = {})
      # UrlProxy.wrap2("/emox/questions/#{id}")
      UrlProxy.wrap2("/training?question_id=#{id}")
      # Rails.application.routes.url_helpers.url_for([:emox, {only_path: false, question_id: id}.merge(options)])
    end
    ...
    #   }
    #   question = user.emox_questions.build
