module Emox::Question::ImportExportMod
  ...
  class_methods do
    ...
    def export_all
      ...
      file = Rails.root.join("app/models/emox/#{name.demodulize.underscore.pluralize}.yml")
    ...
    def import_all(user = User.sysop)
      persistent_records.each do |e|
        record = user.emox_questions.find_or_initialize_by(key: e[:key])
    ...
    def persistent_file
      Rails.root.join("app/models/emox/#{name.demodulize.underscore.pluralize}.yml")
    end
