  ...
  App.single_notification = App.cable.subscriptions.create({channel: "SingleNotificationChannel"}, {
    ...
    received(data) {
      ...
      if (data["message"]) {
        ...
        this.$toast.open({message: str, position: "is-bottom", type: "is-info", duration: 1000 * 3})
        this.talk(message)
      }
...
`
        ...
        this.talk(`${e.from.name}さんからの挑戦状が届きました`)
      ...
      if (data["matching_establish"]) {
        ...
        if (data["auto_matched_at"]) {
          this.$toast.open({message: "マッチングが成立しました", position: "is-bottom", type: "is-info", duration: 1000 * 3})
        }
        if (data["battle_request_at"]) {
          this.$toast.open({message: "申し込みが成立しました", position: "is-bottom", type: "is-info", duration: 1000 * 3})
        }
