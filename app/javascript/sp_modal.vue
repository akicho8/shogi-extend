<template lang="pug">
  ...
  b-modal.sp_modal(:active.sync="new_modal_p" trap-focus animation="" :full-screen="true" :can-cancel="['escape', 'outside']" :has-modal-card="true" v-if="record")
    .modal-card.is-shogi-player-modal-card
      .modal-card-body
        // 自分で閉じるボタン設置。組み込みのはもともとフルスクリーンを考慮しておらず、白地に白いボタンで見えないため。
        .delete.is-large(aria-label="close" @click="new_modal_p = false" v-if="true")
        ...
        sp_modal_time_chart(:record="record" :show_p="time_chart_p" ref="sp_modal_time_chart" @update:turn="turn_set_from_chart" refs="sp_modal_time_chart" :chart_turn="turn_offset" :flip="new_flip")
