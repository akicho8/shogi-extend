export const application_battle = {
  ...
  data() {
    return {
      ...
      battle_code: null,                          // 合言葉
    }
  },
  created() {
    this.battle_code_set(this.info.record.battle_code, {initial: true})
  },
  methods: {
    battle_code_set(battle_code, options = {}) {
      this.battle_code = battle_code
      this.battle_unsubscribe() // 内容が変更になったかもしれないのでいったん解除
      if (this.battle_code) {
        this.battle_setup(options)
      }
    },

    battle_setup(options = {}) {
      // this.$gtag.event("open", {event_category: "対戦中"})

      this.battle_unsubscribe()

      this.__assert__(this.$ac_battle == null, "this.$ac_battle == null")
      this.$ac_battle = this.ac_subscription_create({channel: "ShareBoard::BattleChannel", battle_code: this.battle_code}, {
        connected: () => {
          if (!options.initial) {
            this.ok_notice("合言葉を設定しました")
          }
        },
        disconnected: () => {
          // this.ok_notice("部屋を解放しました")
        },
      })
    },
    ...
    ac_battle_perform(action, params = {}) {
      ...
      console.log(`**→ [${this.user_code}][${action}] ` + JSON.stringify(params))

      this.$ac_battle.perform(action, params) // --> app/channels/share_board/battle_channel.rb
    },
    ...
    //   }) // --> app/channels/share_board/battle_channel.rb
    //
    //   this.ok_notice("対戦開始")
    ...
    play_board_share_broadcasted(params) {
      ...
      if (params.user_code === this.user_code) {
        // this.$buefy.toast.open("自分")
      } else {
        // this.$buefy.toast.open("相手")
        // 自分の操作を相手の盤面で動かす
        this.play_mode_body = params.share_sfen
        this.turn_offset = this.sfen_parse(params.share_sfen).moves.length

        // if (false) {
        //   this.$refs.main_sp.api_sfen_or_kif_set(params.share_sfen, {turn:-1})
        //   this.sound_play("pishi") // shogi-player で音が鳴らないのでここで鳴らす
        // }
        // this.turn_offset = -1

        // alert(params.share_sfen)
        // this.$refs.main_sp.api_sfen_or_kif_set("position sfen 7nl/7k1/9/7pp/6N2/9/9/9/9 b GS2r2b3g3s2n3l16p 1 moves S*2c 2b3c G*4c", {turn:10000})
        // this.$refs.main_sp.api_sfen_or_kif_set("position sfen 7nl/7k1/9/7pp/6N2/9/9/9/9 b GS2r2b3g3s2n3l16p 1 moves S*2c 2b3c G*4c", {turn:10000})
        // this.$refs.main_sp.api_sfen_or_kif_set(params.share_sfen, {turn:10000})
      }
    },
    //
    // ////////////////////////////////////////////////////////////////////////////////
    ...
    //   const mi = this.member_infos_hash[params.user_code]          // 対応する membership の情報
    //
    //   // 効果音
    ...
    //       this.ac_battle_perform("owattayo", {member_infos_hash: this.member_infos_hash}) // --> app/channels/share_board/battle_channel.rb
    //     } else {
    //       this.next_trigger()
    //     }
    //   })
    ...
    //   const mi = this.member_infos_hash[params.user_code]
    //   if (params.user_code === this.user_code) {
    //     // 先に解答ボタンを押した側
    //     this.x_mode = "x2_play"
    ...
    //   this.ac_battle_perform("battle_continue_handle", {user_code: this.user_code})
    // },
    ...
    //   const membership = this.battle.memberships.find(e => e.id === params.user_code)
  ...
  computed: {
    // leader_p() {
    //   return this.battle.memberships[this.app.config.leader_index].id === this.user_code
    // },
    // current_membership() {
    ...
    // },
    // current_mi() {
    //   return this.member_infos_hash[this.user_code]
    // },
    // opponent_mi() {
    ...
    // },
    // b_score_max() {
    //   return _.max(_.map(this.member_infos_hash, (e, user_code) => e.b_score))
    // },
    //
    // // バトル終了条件
    // battle_end_p() {
