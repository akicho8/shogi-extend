...
.the_builder_edit_form
  b-field(label="タイトル" label-position="on-border")
    b-input(v-model.trim="$store.state.builder.question.title" required)
  ...
  b-field(label="解説" label-position="on-border")
    b-input(v-model.trim="$store.state.builder.question.description" type="textarea" rows="3")

  b-field(label="難易度" custom-class="is-small" v-if="app.config.difficulty_level_enable")
    b-rate(v-model="$store.state.builder.question.difficulty_level" spaced :max="app.config.difficulty_level_max" :show-score="false")

  b-field(label="メッセージ" label-position="on-border" message="問題と一緒に表示する文言です。何手指してほしいかや、ヒントを伝えたいときだけ入力してください。基本未入力でかまいません")
    b-input(v-model.trim="$store.state.builder.question.direction_message" placeholder="3手指してください")

  b-field(label="タグ" label-position="on-border")
    //- https://buefy.org/documentation/taginput
    b-taginput(v-model="$store.state.builder.question.owner_tag_list" rounded :confirm-key-codes="[13, 188, 9, 32]")
  ...
  b-collapse.mt-6(:open="source_author_collapse_open_p")
    ...
    .box.py-5.mt-2
      b-field
        b-switch(v-model="$store.state.builder.question.source_about_key" size="is-small" true-value="unknown" false-value="ascertained") 作者不詳

      b-field(label="作者" label-position="on-border")
        b-input(v-model.trim="$store.state.builder.question.source_author" placeholder="初代大橋宗桂" :disabled="$store.state.builder.question.source_about_key === 'unknown'")

      b-field(label="出典メディア" label-position="on-border")
        b-input(v-model.trim="$store.state.builder.question.source_media_name" placeholder="詰パラ")

      b-field(label="出典年月日" label-position="on-border")
        b-datepicker(
          v-model="$store.state.builder.question.date_casted_source_published_on"
