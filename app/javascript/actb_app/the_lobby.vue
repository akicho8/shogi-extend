...
.the_lobby
  ...
  .opening(v-if="app.sub_mode === 'opening'")
    ...
    .buttons.is-centered
      b-button.has-text-weight-bold(@click="app.game_key_select_handle" type="is-primary") START

  .game_key_select(v-if="app.sub_mode === 'game_key_select'")
    .title.is-3.has-text-centered モード
    .mode_buttons.buttons.is-centered
      b-button.has-text-weight-bold(@click="app.game_key_set_handle('game_key1')" type="is-primary") マラソン
      b-button.has-text-weight-bold(@click="app.game_key_set_handle('game_key2')" type="is-primary") シングルトン
