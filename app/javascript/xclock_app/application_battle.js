...
export const application_battle = {
  ...
  methods: {
    ac_battle_perform(action, params = {}) {
      ...
      this.$ac_battle.perform(action, params) // --> app/channels/xclock/battle_channel.rb
    },
    ...
    battle_setup(battle) {
      ...
      this.$ac_battle = this.ac_subscription_create({channel: "Xclock::BattleChannel", battle_id: this.battle.id}, {
        connected: () => {
    ...
    delay_and_judgement_run_or_next_trigger(ox_mark_info) {
      this.delay(ox_mark_info.delay_second, () => {
        if (this.battle_end_p || this.next_question_empty_p) {
          this.ac_battle_perform("judgement_run", {member_infos_hash: this.member_infos_hash}) // --> app/channels/xclock/battle_channel.rb
        } else {
          this.next_trigger()
        }
      })
