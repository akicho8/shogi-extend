        ...
        list.each do |history_one|
          unless BattleRecord.where(battle_key: history_one[:battle_key]).exists?
            ...
            history_one[:users].each do |e|
              unless BattleUser.where(user_key: e[:user_key]).exists?
                BattleUser.create!(user_key: e[:user_key], user_rank: e[:rank])
              end
            end
            ...
            battle_users = history_one[:users].collect { |e|
              BattleUser.find_by!(user_key: e[:user_key])
            }

            battle_record.battle_users = battle_users
          end
        end
      end
