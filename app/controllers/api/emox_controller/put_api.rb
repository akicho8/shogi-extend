  ...
  class EmoxController
    concern :PutApi do
      ...
      def new_challenge_accept_handle
        ...
        rule = Emox::Rule.fetch(params[:rule_key])
        ...
        if Emox::BaseChannel.once_run(key, expires_in: 3.seconds)
          current_user.emox_setting.update!(session_lock_token: params[:session_lock_token])
          Emox::Room.create_with_members!([user, current_user], rule: rule)
      ...
      #   raise ArgumentError if params[:session_lock_token].blank?
      #
      #   ids = Emox::Rule.matching_all_user_ids
      ...
      #   user = User.find(id)
      #   current_user.emox_setting.update!(session_lock_token: params[:session_lock_token])
      #   Emox::Room.create_with_members!([user, current_user], rule: user.emox_setting.rule)
      #   { status: "success" }
      # end
      ...
      def question_save_handle
        if id = params[:question][:id]
          question = Emox::Question.find(id)
        else
          question = current_user.emox_questions.build
        end
        begin
      ...
      def emotion_save_handle
        ...
        { emotions: current_user.emotions.reload.as_json(Emox::Emotion.json_type13) }
      end
      ...
      def emotions_import_handle
        Emox::EmotionInfo.each do |e|
          folder = Emox::EmotionFolder.fetch(e.folder_key)
          current_user.emotions.create(folder: folder, name: e.name, message: e.message, voice: e.voice)
        end
        { emotions: current_user.emotions.reload.as_json(Emox::Emotion.json_type13) }
      end

      # # curl -d _method=put -d user_name=a -d remote_action=user_profile_update -d _user_id=1 http://localhost:3000/api/emox
      ...
      # from app/javascript/emox_app/the_profile_edit_form.vue profile_update_handle
