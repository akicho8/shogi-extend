module Api
  class WbookController
    concern :PutApi do
      ...
      # curl -d _method=put -d _user_id=1 -d remote_action=session_lock_token_set_handle -d session_lock_token=xxx http://localhost:3000/api/wbook.json
      def session_lock_token_set_handle
        raise ArgumentError, params.inspect if params[:session_lock_token].blank?
        # if current_user.wbook_setting.session_lock_token
        #   return { error_message: "" }
        # end
        current_user.create_wbook_setting_if_blank
        current_user.wbook_setting.update!(session_lock_token: params[:session_lock_token])
        { status: "success" }
      end

      # curl -d _method=put -d _user_id=1 -d remote_action=wbook_session_lock_token_valid_handle -d session_lock_token=xxx http://localhost:3000/api/wbook.json
      def wbook_session_lock_token_valid_handle
        raise ArgumentError, params.inspect if params[:session_lock_token].blank?
        if current_user.wbook_session_lock_token_valid?(params[:session_lock_token])
          { status: "success" }
        else
      ...
      # curl -d _method=put -d _user_id=1 -d remote_action=session_lock_token_reset_handle http://localhost:3000/api/wbook.json
      # def session_lock_token_reset_handle
      #   current_user.wbook_setting.update!(session_lock_token: nil)
      #   { status: "success" }
      # end

      # curl -d _method=put -d _user_id=1 -d remote_action=rule_key_set_handle -d rule_key=sy_marathon http://localhost:3000/api/wbook.json
      def rule_key_set_handle
        raise ArgumentError, params.inspect if params[:session_lock_token].blank?
        unless current_user.wbook_session_lock_token_valid?(params[:session_lock_token])
          return { status: "session_lock_token_invalid" }
        end
        current_user.wbook_setting.update!(rule: Wbook::Rule.fetch(params[:rule_key]))
        { status: "success" }
      end
      ...
      def debug_matching_add_handle
        if user = User.where.not(id: params[:exclude_user_id]).first
          Wbook::Rule.matching_users_delete_from_all_rules(user)
          if rule_key = params[:rule_key]
            rule = Wbook::Rule.fetch(rule_key)
            user.wbook_setting.update!(rule: rule)
          else
            rule = current_user.wbook_setting.rule
          end
      ...
      def matching_users_clear_handle
        Wbook::Rule.matching_users_clear
        { status: "success" }
      end
      ...
      def new_challenge_accept_handle
        ...
        rule = Wbook::Rule.fetch(params[:rule_key])
        ...
        if Wbook::BaseChannel.once_run(key, expires_in: 3.seconds)
          current_user.wbook_setting.update!(session_lock_token: params[:session_lock_token])
          Wbook::Room.create_with_members!([user, current_user], rule: rule)
      ...
      #   raise ArgumentError if params[:session_lock_token].blank?
      #
      #   ids = Wbook::Rule.matching_all_user_ids
      ...
      #   user = User.find(id)
      #   current_user.wbook_setting.update!(session_lock_token: params[:session_lock_token])
      #   Wbook::Room.create_with_members!([user, current_user], rule: user.wbook_setting.rule)
      #   { status: "success" }
      # end
      ...
      def emotion_save_handle
        ...
        { emotions: current_user.emotions.reload.as_json(Wbook::Emotion.json_type13) }
      end
      ...
      def emotions_reset_handle
        current_user.wbook_emotions_setup(reset: true)
        { emotions: current_user.emotions.reload.as_json(Wbook::Emotion.json_type13) }
      end
      ...
      def emotions_import_handle
        Wbook::EmotionInfo.each do |e|
          folder = Wbook::EmotionFolder.fetch(e.folder_key)
          current_user.emotions.create(folder: folder, name: e.name, message: e.message, voice: e.voice)
        end
        { emotions: current_user.emotions.reload.as_json(Wbook::Emotion.json_type13) }
      end

      # # curl -d _method=put -d user_name=a -d remote_action=user_profile_update -d _user_id=1 http://localhost:3000/api/wbook
      ...
      # from app/javascript/wbook_app/the_profile_edit_form.vue profile_update_handle
