  ...
  class WbookController
    concern :DebugMod do
      ...
      def debug_for_result(info)
        ...
        users.each do |user|
          record = user.wbook_main_xrecord
          record.skill_key = "A+"
          record.skill_point = 50
          record.save!
        end

        record = users[0].wbook_main_xrecord
        record.skill_add(60)
        record.save!

        record = users[1].wbook_main_xrecord
        record.skill_add(60)
        record.save!

        room = Wbook::Room.create_with_members!(users)
        battle = room.battle_create_with_members!(final: Wbook::Final.fetch(:f_disconnect))
        battle.memberships[0].update!(judge: Wbook::Judge.fetch(:win))
        battle.memberships[1].update!(judge: Wbook::Judge.fetch(:lose))
        battle.reload
      ...
      def debug_for_ov_user_info(info)
        ...
        e = current_user.wbook_main_xrecord
        e.win_count = 1
        e.lose_count = 2
        e.save!
      ...
      def debug_for_battle_sy(info, rule_key)
        ...
        room = Wbook::Room.create_with_members!(users, rule: rule)
