module Api
  class ActbController
    concern :GetApi do
      ...
      # 問題一覧
      # http://localhost:3000/api/actb.json?remote_action=questions_fetch
      # http://localhost:3000/api/actb.json?remote_action=questions_fetch&folder_key=active
      # http://localhost:3000/api/actb.json?remote_action=questions_fetch&sort_column=lineage_key&sort_order=desc
      ...
      # http://localhost:3000/api/actb.json?remote_action=ranking_fetch&ranking_key=rating
      def ranking_fetch
        { rank_data: Actb::RankingCop.new(params.merge(current_user: current_user)) }
      end

      # http://localhost:3000/api/actb.json?remote_action=seasons_fetch
      def seasons_fetch
        { seasons: Actb::Season.newest_order.as_json(only: [:id, :generation, :name, :begin_at, :end_at]) }
      end

      # http://localhost:3000/api/actb.json?remote_action=history_records_fetch
      ...
      # http://localhost:3000/api/actb.json?remote_action=clip_records_fetch
      ...
      # http://localhost:3000/api/actb.json?remote_action=question_single_fetch&question_id=1
      ...
      # http://localhost:3000/api/actb.json?remote_action=resource_fetch
      def resource_fetch
        {
      ...
      # http://localhost:3000/api/actb.json?remote_action=builder_form_resource_fetch
      ...
      # http://localhost:3000/api/actb.json?remote_action=lobby_messages_fetch
      ...
      # http://localhost:3000/api/actb.json?remote_action=notifications_fetch
      ...
      # http://localhost:3000/api/actb.json?remote_action=revision_fetch
      def revision_fetch
        { revision: Actb::Config[:revision] }
      end

      # 作成した問題数を返す
      # http://localhost:3000/api/actb.json?remote_action=zip_dl_count_fetch
