...
module Api
  class ProfessionalsController < ::Api::ApplicationController
    ...
    def rows
      user_stats_hash = user_stats_fetch.inject({}) { |a, e| a.merge(e[:key].downcase => e) }
      ...
      users = users.collect do |user|
        {}.tap do |row|
          name = user.key
          if user_stat = user_stats_hash[user.key.downcase]
            if s = user_stat["名前"].to_s.remove(/\s*\<.*?\>/).presence
              name = s
            end
          end
