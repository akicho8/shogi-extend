  ...
  class BattleChannel < BaseChannel
    ...
    def speak(data)
      ...
      if data[:message_body].start_with?("/")
        ...
        current_user.emox_room_messages.create!(body: data[:message_body], room: current_battle.room)
      end
    end
    ...
    def kotae_sentaku(data)
      ...
      ox_mark = Emox::OxMark.fetch(data[:ox_mark_key])                      # 結果
    ...
    def history_create(user, question, ox_mark)
      if current_battle.room.bot_user == user
        ...
        user.emox_histories.create!(question: question, ox_mark: OxMark.fetch(ox_mark), room: current_battle.room)
      end
    end
