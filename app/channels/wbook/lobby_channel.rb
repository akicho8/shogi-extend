module Wbook
  class LobbyChannel < BaseChannel
    ...
    def subscribed
      ...
      stream_from "wbook/lobby_channel"
      Wbook::Rule.matching_user_ids_broadcast
    end
    ...
    # from app/javascript/wbook_app/application_matching.js
    def matching_search(data)
      ...
      current_user.reload # current_user.wbook_setting.* を最新にするため

      # session_lock_token が変化していたら別のブラウザで対戦が開始されたことがわかる
      unless current_user.wbook_session_lock_token_valid?(data[:session_lock_token])
        ActionCable.server.broadcast("wbook/lobby_channel", bc_action: :session_lock_token_invalid_narrowcasted, bc_params: data)
        return
      end
