module ShareBoard
  class BattleChannel < BaseChannel
    def subscribed
      raise ArgumentError, params.inspect unless battle_code

      stream_from "share_board/battle_channel/#{battle_code}"
    end

    # # バトルが正常終了していない状態で切断された場合に負け
    def unsubscribed
      # if current_battle.end_at.blank?
      #   if once_run("share_board/battles/#{current_battle.id}/disconnect")
      #     say "*切断しました"
    ...
    #   else
    #     current_user.share_board_room_messages.create!(body: data[:message_body], room: current_battle.room)
    #   end
    # end
    #
    ...
    #   data = data.to_options
    #   ox_mark = ShareBoard::OxMark.fetch(data[:ox_mark_key])
    ...
    #     current_user.share_board_histories.find_or_initialize_by(question: question, membership: my_membership).update!(ox_mark: ox_mark)
    ...
    #       my_membership.user.share_board_histories.find_or_initialize_by(membership: my_membership, question: question).update!(ox_mark: ox_mark)
    #       op_membership.user.share_board_histories.find_or_initialize_by(membership: op_membership, question: question).update!(ox_mark: ShareBoard::OxMark.fetch(:mistake))
    ...
    #         membership.user.share_board_histories.find_or_initialize_by(question: question, membership: membership).update!(ox_mark: ox_mark)
    #       end
    ...
    #     question = Question.find(data[:question_id])
    #     current_battle.memberships.each do |membership|
    #       membership.user.share_board_histories.find_or_initialize_by(question: question, membership: membership).update!(ox_mark: OxMark.fetch(:mistake))
    #     end
    #   end
    #
    #   bc_params = {
    ...
    #
    # # <-- app/javascript/share_board_app/application.vue
    # def goal_hook(data)
    #   judge_final_set(current_user, :win, :f_success)
    # end
    #
    ...
    #   if b_scores.max < ShareBoard::Config[:b_score_max_for_win]
    ...
    #     membership = ShareBoard::BattleMembership.find(data[:membership_id])
    ...
    #   question = Question.find(data[:question_id])
    #   membership = current_battle.memberships.find(data[:membership_id])
    #   history = current_user.share_board_histories.find_or_initialize_by(question: question, membership: membership)
    #   history.update!(ox_mark: ShareBoard::OxMark.fetch(ox_mark))
    # end
    #
    def broadcast(bc_action, bc_params)
      raise ArgumentError, bc_params.inspect unless bc_params.values.all?
      ActionCable.server.broadcast("share_board/battle_channel/#{battle_code}", {bc_action: bc_action, bc_params: bc_params})
    end
    #
