module ShareBoard
  class LobbyChannel < BaseChannel
    ...
    def subscribed
      ...
      ShareBoard::Rule.matching_user_ids_broadcast
    end
    ...
    # from app/javascript/share_board_app/the_matching_interval.js
    ...
    def room_create(*users)
      users.each { |e| ShareBoard::Rule.matching_users_delete_from_all_rules(e) }

      # app/models/share_board/room.rb
    ...
    def rule
      current_user.share_board_setting.rule
    end
    delegate :matching_users, :matching_user_ids, to: :rule

    def lobby_speak(message_body)
      current_user.share_board_lobby_messages.create!(body: message_body)
    end
  end
end
