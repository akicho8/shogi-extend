class BattleChannel < ApplicationCable::Channel
  ...
  def play_mode_long_sfen_set(data)
    ...
    begin
      ...
      current_battle.update!(end_at: Time.current, win_location_key: current_location.flip.key, last_action_key: "ILLEGAL_MOVE")
      game_end_broadcast
      return
    end
    ...
    if mediator.opponent_player.mate_danger?
      ...
      current_battle.update!(end_at: Time.current, win_location_key: mediator.current_player.location.key, last_action_key: "ILLEGAL_MOVE")
      game_end_broadcast
      return
    end
    ...
    current_battle.clock_counts[mediator.opponent_player.location.key].push(data["clock_counter"].to_i) # push でも AR は INSERT 対象になる
    current_battle.kifu_body_sfen = kifu_body_sfen
    current_battle.turn_max = mediator.turn_info.turn_max
    current_battle.save!

    broadcast_data = {
      turn_max: mediator.turn_info.turn_max,
      kifu_body_sfen: kifu_body_sfen,
      human_kifu_text: current_battle.human_kifu_text,
      last_hand: ki2_a.last,
      moved_user_id: current_user.id, # 操作した人(この人以外に盤面を反映する)
      clock_counts: current_battle.clock_counts,
    }
    ...
    if hands.empty?
      current_battle.update!(end_at: Time.current, win_location_key: mediator.opponent_player.location.key, last_action_key: "TSUMI")
      game_end_broadcast
      return
    end
  end
  ...
  def chat_say(data)
    chat_message = current_user.chat_messages.create!(battle: current_battle, message: data["message"])
  ...
  def room_in(data)
    ...
    # どの部屋にいるか設定
    current_user.update!(current_battle: current_battle)

    # 部屋のメンバーとして登録(マッチング済みの場合はもう登録されている)
    # unless current_battle.users.include?(current_user)
    #   current_battle.users << current_user
    # end
    ...
    if current_memberships.present?
      ...
      # 観戦者
      unless current_battle.watch_users.include?(current_user)
        current_battle.watch_users << current_user
        current_battle.broadcast # counter_cache の watch_ships_count を反映させるため
      end
    end

    if current_battle.end_at
    else
      # 自分が対局者の場合
      if current_memberships.present?
        if current_memberships.all? { |e| e.standby_at }
          ...
          if current_battle.memberships.standby_enable.count >= current_battle.memberships.count
            battle_start({})
          end
        end
      end
  ...
  def room_out(data)
    ...
    current_user.update!(current_battle_id: nil)
    ...
    if current_memberships.present?
      ...
      # 観戦者
      current_battle.watch_users.destroy(current_user)
      # ActionCable.server.broadcast(room_key, watch_users: current_battle.watch_users)
    end
  ...
  def battle_start(data)
    current_battle.update!(begin_at: Time.current)
    ActionCable.server.broadcast(room_key, {
        begin_at: current_battle.begin_at,
        human_kifu_text: current_battle.human_kifu_text, # 開始日時が埋められた棋譜で更新したいため
      })
  end

  def time_up_trigger(data)
    # membership_ids は送ってきた人で対応するレコードにタイムアップ認定する
    memberships = current_battle.memberships.where(id: data["membership_ids"])
    ...
    if current_battle.memberships.where.not(time_up_trigger_at: nil).count >= current_battle.memberships.count
      current_battle.update!(end_at: Time.current, win_location_key: data["win_location_key"], last_action_key: "TIME_UP")
      game_end_broadcast
    end
  end
  ...
  def give_up_trigger(data)
    current_battle.update!(end_at: Time.current, win_location_key: data["win_location_key"], last_action_key: "TORYO")
    game_end_broadcast
  end

  # 先後をまとめて反転する
  def location_flip_all(data)
    current_battle.memberships.each(&:location_flip!)
    memberships_update
  end

  def game_end_broadcast
    ActionCable.server.broadcast(room_key, {
        end_at: current_battle.end_at,
        win_location_key: current_battle.win_location_key,
        last_action_key: current_battle.last_action_key,
        human_kifu_text: current_battle.human_kifu_text, # end_at が埋めこまれた棋譜で更新しておくため
      })
  end
  ...
  def countdown_mode_on(data)
    ...
    current_battle.countdown_mode_hash[location.key] = true
    current_battle.save!
  end
  ...
  def current_battle
    @current_battle ||= Fanta::Battle.find(params[:battle_id])
  end

  # 自分対自分の場合もあるためメンバー情報は複数ある
  def current_memberships
    @current_memberships ||= current_battle.memberships.where(user: current_user)
  end
end
