module Xclock
  class RoomChannel < BaseChannel
    ...
    def subscribed
      ...
      stream_from "xclock/room_channel/#{room_id}"
      self.class.active_users_add(current_user)
      debug_say "*入室しました"

      if once_run("xclock/rooms/#{current_room.id}/first_battle_create")
        battle = current_room.battle_create_with_members!
        debug_say "**最初のバトル作成(id:#{battle.id})"
        # --> app/jobs/xclock/battle_broadcast_job.rb --> battle_broadcasted --> app/javascript/xclock_app/application_room.js
      end
    end
    ...
    def unsubscribed
      ...
      if current_room.end_at.blank?
        if once_run("xclock/rooms/#{current_room.id}/disconnect")
    ...
    def broadcast(bc_action, bc_params)
      ...
      ActionCable.server.broadcast("xclock/room_channel/#{room_id}", {bc_action: bc_action, bc_params: bc_params})
    end
