module Xclock
  class LobbyChannel < BaseChannel
    ...
    def subscribed
      ...
      stream_from "xclock/lobby_channel"
      Xclock::Rule.matching_user_ids_broadcast
    end

    def unsubscribed
      __event_notify__(__method__)
      Xclock::Rule.matching_users_delete_from_all_rules(current_user)
    end
    ...
    # from app/javascript/xclock_app/application_matching.js
    def matching_search(data)
      ...
      current_user.reload # current_user.xclock_setting.* を最新にするため

      # session_lock_token が変化していたら別のブラウザで対戦が開始されたことがわかる
      unless current_user.session_lock_token_valid?(data[:session_lock_token])
        ActionCable.server.broadcast("xclock/lobby_channel", bc_action: :session_lock_token_invalid_narrowcasted, bc_params: data)
        return
      end
