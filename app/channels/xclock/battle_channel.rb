module Xclock
  class BattleChannel < BaseChannel
    ...
    def subscribed
      ...
      stream_from "xclock/battle_channel/#{battle_id}"
    end
    ...
    def unsubscribed
      ...
      if current_battle.end_at.blank?
        if once_run("xclock/battles/#{current_battle.id}/disconnect")
          say "*切断しました"
    ...
    def speak(data)
      ...
      if data[:message_body].start_with?("/")
        ...
        current_user.xclock_room_messages.create!(body: data[:message_body], room: current_battle.room)
      end
    end
    ...
    def kotae_sentaku(data)
      ...
      ox_mark = Xclock::OxMark.fetch(data[:ox_mark_key])                      # 結果
    ...
    def history_create(user, question, ox_mark)
      if current_battle.room.bot_user == user
        ...
        user.xclock_histories.create!(question: question, ox_mark: OxMark.fetch(ox_mark), room: current_battle.room)
      end
    end
    ...
    def broadcast(bc_action, bc_params)
      ...
      ActionCable.server.broadcast("xclock/battle_channel/#{battle_id}", {bc_action: bc_action, bc_params: bc_params})
    end
