...
describe("TurnProgress", () => {
  ...
  test("相対的に下げる", () => {
    ...
    expect(turn_progress.turn_diff).toEqual(-2)
    expect(turn_progress.turn_step).toEqual(2)
    expect(turn_progress.turn_next_p).toEqual(false)
    expect(turn_progress.turn_previous_p).toEqual(true)
    expect(turn_progress.turn_same_p).toEqual(false)
  ...
  test("相対的に上げる", () => {
    ...
    expect(turn_progress.turn_diff).toEqual(2)
    expect(turn_progress.turn_step).toEqual(2)
    expect(turn_progress.turn_next_p).toEqual(true)
  ...
  test("マイナスにはならない", () => {
    ...
    expect(turn_progress.turn_diff).toEqual(-5)
    expect(turn_progress.turn_step).toEqual(5)
    expect(turn_progress.turn_next_p).toEqual(false)
    expect(turn_progress.turn_previous_p).toEqual(true)
    expect(turn_progress.turn_same_p).toEqual(false)
  ...
  test("変化しない", () => {
    ...
    expect(turn_progress.turn_diff).toEqual(0)
    expect(turn_progress.turn_step).toEqual(0)
    expect(turn_progress.turn_next_p).toEqual(false)
    expect(turn_progress.turn_previous_p).toEqual(false)
  ...
  describe("#sfen_go_back_p", () => {
    test("過去の棋譜", () => {
      ...
      expect(turn_progress.sfen_go_back_p).toEqual(true)
    ...
    test("別の世界線の棋譜", () => {
      ...
      const turn_progress = TurnProgress.create({old_sfen, old_turn: 2, new_sfen, to: 1})
      expect(turn_progress.sfen_go_back_p).toEqual(false)
