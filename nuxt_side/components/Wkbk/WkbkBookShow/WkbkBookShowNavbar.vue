...
MainNavbar.WkbkBookShowNavbar(v-bind="component_attrs")
  template(v-if="base.book")
    ...
    template(slot="end")
      template(v-if="base.is_standby_p")
        b-navbar-item.px_5_if_tablet(tag="nuxt-link" :to="{name: 'rack-articles-new', query: {book_key: base.book.key}}"        @click.native="sfx_click()" v-if="base.owner_p")
          b-icon(icon="plus")
        b-navbar-item.px_5_if_tablet(tag="nuxt-link" :to="{name: 'rack-books-book_key-edit', params: {book_key: base.book.key}}" @click.native="sfx_click()" v-if="base.owner_p")
          b-icon(icon="pencil")
