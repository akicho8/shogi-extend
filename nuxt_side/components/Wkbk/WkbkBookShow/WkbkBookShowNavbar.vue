  ...
  template(v-if="base.book")
    ...
    template(slot="start")
      ...
      template(v-if="base.is_running_p")
        ...
        //- .slice().reverse()
        template(v-if="false")
          template(v-for="e in ary_reverse(base.AnswerKindInfo.values)")
            b-navbar-item.has-text-weight-bold.px-5.is-clickable.is-unselectable(@click="base.next_handle(e)" v-if="base.current_xitem")
              b-icon(:icon="e.icon")

    template(slot="end")
      template(v-if="base.is_standby_p")
        b-navbar-item.px_5_if_tablet(tag="nuxt-link" :to="{name: 'rack-articles-new', query: {book_key: base.book.key}}"        @click.native="$sound.play_click()" v-if="base.owner_p")
          b-icon(icon="plus")
        b-navbar-item.px_5_if_tablet(tag="nuxt-link" :to="{name: 'rack-books-book_key-edit', params: {book_key: base.book.key}}" @click.native="$sound.play_click()" v-if="base.owner_p")
          b-icon(icon="pencil")
