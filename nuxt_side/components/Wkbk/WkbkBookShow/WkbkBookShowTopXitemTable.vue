...
nav.panel.mb-0.WkbkBookShowTopXitemTable
  ...
  .panel-block.op_buttons
    ...
    //-   | 元に戻す
  .panel-block
    b-numberinput(controls-position="compact" expanded v-model="base.current_index_human" :min="1" :max="base.xitems.length" :exponential="true" @click.native="sound_play_click()")
    //- b-button.is-fullwidth(@click="base.op_select_x_handle")
    //-   | 不正解のみ残す
  .panel-block.is-block
    b-table(
      ...
      // :has-detailed-visible="row => sound_play_click()"
      ...
      b-table-column(v-slot="{row}" custom-key="title" field="article.title" label="問題" cell-class="is_line_break_on" sortable)
        ...
        //- nuxt-link.article_title(:to="{name: 'rack-articles-article_key', params: {article_key: row.key}}" @click.native="sound_play_click()")
      ...
      //-     nuxt-link(:to="{name: 'rack-articles-article_key-edit', params: {article_key: row.key}}" @click.native="sound_play_click()") 編集

      //- b-table-column(v-slot="{row}" custom-key="operation" label="" :width="1")
      //-   b-dropdown.is-pulled-right(position="is-bottom-left" :close-on-click="false" :mobile-modal="false" @active-change="sound_play_click()" @click.native.prevent)
