  ...
  b-table(
    ...
    b-table-column(v-slot="{row}" custom-key="operation" label="" :width="1" cell-class="operation_block")
      .buttons.is-centered.mb-0(v-if="g_current_user && g_current_user.id === row.user.id || development_p")
        nuxt-link.button.is-small.mb-0(:to="{name: 'rack-articles-article_key-edit', params: {article_key: row.key}}" @click.native="sound_play_click()") 編集
        nuxt-link.button.is-small.mb-0(:to="{name: 'rack-articles-new', query: {source_article_key: row.key}}" @click.native="sound_play_click()") コピーして新規
