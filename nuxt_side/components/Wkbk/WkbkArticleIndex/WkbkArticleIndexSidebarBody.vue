...
.WkbkArticleIndexSidebarBody
  b-menu
    ...
    //-   b-menu-item.is_active_unset(tag="nuxt-link" :to="{name: 'rack-articles-new'}" label="問題作成" @click.native="$sound.play_click()")
    ...
    b-menu-list(label="表示オプション")
      b-menu-item.is_active_unset.sidebar_columns_toggle(:disabled="base.display_option_disabled" @click="$sound.play_click()")
      ...
      b-menu-item.is_active_unset(@click="$sound.play_click()" :disabled="base.display_option_disabled")
        template(slot="label" slot-scope="props")
          | 図面表示
          b-icon.is-pulled-right(:icon="props.expanded ? 'menu-up' : 'menu-down'")
      ...
      b-menu-item.is_active_unset(:disabled="base.display_option_disabled" v-if="development_p")
        template(slot="label")
          span 表示カラム
          b-dropdown.is-pulled-right(position="is-bottom-left" :close-on-click="false" :mobile-modal="false" @active-change="$sound.play_click()")
