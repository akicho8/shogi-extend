    ...
    .mt-4
      ...
      b-menu
        ...
        b-menu-list(label="対局サポート")
          ...
          b-menu-item.is_active_unset(__unuse__icon="home" label="対局履歴" @click="SB.general_dashboard_modal_handle" :disabled="$GX.blank_p(SB.ac_room)")
          b-menu-item.is_active_unset(__unuse__icon="trophy" tag="nuxt-link" label="対局履歴(nuxt-link)" :to="{name: 'share-board-dashboard', query: {room_key: SB.room_key}}" @click.native="sfx_click()" :disabled="$GX.blank_p(SB.ac_room)" v-if="development_p")
          b-menu-item.is_active_unset(__unuse__icon="trophy" label="対局履歴(hrefで別タブ)" :href="SB.dashboard_url" target="_blank" :disabled="$GX.blank_p(SB.ac_room)" v-if="development_p")
