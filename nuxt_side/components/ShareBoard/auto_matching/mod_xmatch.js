...
const START_TOAST_DELAY = 3                // 誰々から指してくださいをN秒後に発動する
...
export const mod_xmatch = {
  ...
  methods: {
    ...
    test_direct1() {
      ...
      this.toast_primary(`${this.user_call_name(this.current_turn_user_name)}から指してください`)
    },
    ...
    async xmatch_establishment(params) {
      ...
      this.xmatch_setup1_member(params)     // 対局設定
    ...
    xmatch_setup1_member(params) {
      ...
      this.tl_add("対局設定", names, this.order_flow)
    },
    // 手合割と視点設定
    xmatch_setup2_handicap(params) {
      const xmatch_rule_info = XmatchRuleInfo.fetch(params.xmatch_rule_key)
      this.current_sfen_set({sfen: xmatch_rule_info.board_preset_info.sfen, turn: 0})
