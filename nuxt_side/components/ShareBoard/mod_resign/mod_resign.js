  ...
  methods: {
    ...
    resign_call(options = {}) {
      ...
      const ending_context_default_params = {
        ...
        teams_hash: this.player_names_from_member,      // この時点のメンバー情報を持っておく (あとでやるとルール設定がOFFになっているか気にかけないといけない)
      }
      ...
      this.battle_save_by_win_location(options.win_location_key) // ルール設定がある状態で対局を保存する
      this.resign_share(options)                                 // 最後にルール設定を解除する
    },
    ...
    // 引分
    draw_call() {
      this.battle_save_by_win_location(null)                // ルール設定がある状態で対局を保存する
      this.resign_share({ending_route_key: "er_auto_draw"}) // 最後にルール設定を解除する
    },
