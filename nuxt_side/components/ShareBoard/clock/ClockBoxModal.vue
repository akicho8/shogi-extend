...
.modal-card
  ...
  .modal-card-foot
    ...
    template(v-if="SB.clock_box")
      ...
      .buttons
        template(v-if="SB.clock_box.pause_or_play_p && !SB.clock_box.timer")
        ...
        template(v-if="SB.clock_box.pause_or_play_p && !SB.clock_box.timer")
          b-button.resume_button(@click="SB.cbm_resume_handle" icon-left="play-pause")
        template(v-if="!SB.clock_box.pause_or_play_p")
          b-button.play_button(@click="SB.cbm_play_handle" icon-left="play") 対局開始
</template>
...
export default {
  ...
  data() {
    return {
      clock_box_p: !!this.SB.clock_box, // b-switch 用
    }
  },
}
</script>
