...
export const mod_clock_box_modal = {
  ...
  methods: {
    ...
    cbm_play_handle() {
      ...
      // // いったん初期配置に戻すか聞く
      // // 二歩で対局が終わって、再度バトル設定と時計を指定して再開するケースもあるため、これはない方がよい
    ...
    cbm_pause_handle() {
      ...
      //   this.$GX.delay_block(2.5, () => this.toast_primary("続けて検討する場合はバトル設定を切ろう"))
      // }
    },
    cbm_stop_handle() {
      this.sfx_click()
    ...
    cc_play_validate_message() {
      if (this.ac_room) {
        ...
        if (this.order_flat_uniq_users_count === 0) {
          return "先にバトル設定で対局者を指定しよう"
        }
      }
    },
    // cc_order_off_p() { return this.ac_room && !this.order_enable_p }, // バトル設定を有効にしてないのに時計を開始しようとしている？
    // cc_order_users_none_p() { return this.ac_room && this.order_enable_p && this.order_flat_uniq_users_count === 0 }, // バトル設定で誰も設定されていない
  },
}
