...
.ShareBoardHelpBody
  .content
    ...
    article
      ...
      .article_body
        ul
          li 「0手目」まで戻り、全員に盤を転送する
          li 「平手の初期配置をセットする」の<b>ではない</b>

    article
      h2 「初期配置に戻す」で平手の初期配置に戻らないんだが？
      .article_body
        p 盤面編集したのが原因。盤面編集すると確定した局面が0手目になる。「初期配置に戻す」は0手目に移動する動作なので、正常に0手目になっていても「平手の初期配置」を想定している場合に戻ってないと受けとめてしまう
    ...
    article
      ...
      .article_body
        ul
          ...
          li 「1手戻す」
          li 「局面の転送」
        p 元々は上記だけだったがそれ以外は必要に応じて「局面の転送」をしないと同期されないのが理解されないので次のタイミングでも自動的に同期するようにした
        ul
          li 「手合割」適用時
          li 「棋譜の読み込み」した直後
          li 「局面編集」完了時
