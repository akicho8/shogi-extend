...
.SbDebugOrder.columns.is-multiline
  ...
  .column.is-4
    .panel
      .panel-heading
        | 順番操作
      template(v-if="SB.order_draft.order_flow")
        .panel-block {{SB.order_draft.order_flow.inspect}}
  .column.is-4
    .panel
      .panel-heading
        | 名前から引くハッシュ
  ...
  .column.is-4
    .panel
      ...
      .panel-block 自分vs自分で対戦している？ {{SB.dual_role_available_p}}
  ...
  .column.is-4(v-if="SB.order_draft.order_flow")
    .panel
      .panel-heading
        | order_draft stringify
      .panel-block
        pre
          | {{SB.order_draft.order_flow}}
  .column.is-4(v-if="SB.order_draft.order_flow")
    .panel
      .panel-heading
        | order_draft attributes
      .panel-block
        pre
          | {{SB.order_draft.order_flow.attributes}}
</template>
