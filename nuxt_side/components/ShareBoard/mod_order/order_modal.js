...
export const order_modal = {
  data() {
    return {
      // ローカルのモーダルで使うテンポラリ変数
      // 「適用」してはじめて実変数に反映する
      // order_draft はルール設定モーダル用という意味がわかりやすいようにしているだけで特別効果はない
      order_draft: {
  ...
  methods: {
    ...
    order_draft_publish_broadcasted(params) {
      ...
      // ルール設定モーダルを開いているかどうかに関係なくモーダルで使う変数を更新する
    ...
    os_member_delete(user_name) {
      ...
      this.order_draft_publish(`ルール設定から${this.user_call_name(user_name)}を外しました`) // order_draft を配って更新する
    },
  },
  computed: {
