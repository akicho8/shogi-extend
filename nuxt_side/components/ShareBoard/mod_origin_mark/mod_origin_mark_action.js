...
export const mod_origin_mark_action = {
  methods: {
    ev_action_origin_mark_jump_invoke(origin_mark_pos_key, ev) {
      this.origin_mark_share("push", origin_mark_pos_key)
    },

    ev_action_origin_mark_jump_cancel(origin_mark_pos_key, ev) {
      this.origin_mark_share("remove", origin_mark_pos_key)
    },

    //////////////////////////////////////////////////////////////////////////////// 共有

    origin_mark_share(method, origin_mark_pos_key) {
      if (!this.origin_mark_feature_p) {
        return
      }
      const params = {
        __standalone_mode__: true,
        origin_mark_command: GeneralMarkCollection.command_create(method, this.general_mark_attributes_create(origin_mark_pos_key)),
      }
      this.ac_room_perform("origin_mark_share", params) // --> app/channels/share_board/room_channel.rb
    },
    origin_mark_share_broadcasted(params) {
      if (!this.i_can_origin_mark_receive_p(params)) {
        return
      }
      this.sp_call(e => {
        if (e.mut_origin_mark_collection.command_execute$(params.origin_mark_command)) {
