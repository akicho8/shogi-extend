      ...
      b-field(:label="form_part.label" custom-class="is-small" :message="form_part.help_message")
        ...
        template(v-else-if="form_part.type === 'file'")
          b-field(class="file" :class="{'has-name': !!QS.attributes[form_part.key]}")
            b-upload(@input="file => QS.file_upload_handle(form_part, file)" class="file-label")
              span(class="file-cta")
        ...
        template(v-else-if="form_part.type === 'select'")
          b-select(
            ...
            template(v-for="[label, value] in QS.label_value_array(form_part.elems)")
              option(:value="value") {{label}}
        template(v-else-if="form_part.type === 'radio_button' || form_part.type === 'checkbox_button'")
          template(v-for="[label, value] in QS.label_value_array(form_part.elems)")
            component(:is="QS.type_to_component(form_part)" v-model="QS.attributes[form_part.key]" :native-value="value")
              span {{label}}
        template(v-else)
