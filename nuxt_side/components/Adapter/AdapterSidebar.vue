    ...
    .mt-4
      b-menu
        ...
        b-menu-list(label="Export")
          ...
          b-menu-item.is_active_unset(:expanded="false" @click="$sound.play_click()")
          ...
          b-menu-item.is_active_unset(@click="$sound.play_click()")
            template(slot="label" slot-scope="props")
              | コピー
              b-icon.is-pulled-right(:icon="props.expanded ? 'menu-up' : 'menu-down'")
            template(v-for="e in base.FormatTypeInfo.values")
        ...
        b-menu-list(label="その他")
          b-menu-item.is_active_unset(label="ブックマークレット" tag="nuxt-link" :to="{name: 'adapter-bookmarklet'}" @click.native="$sound.play_click()")
          b-menu-item.is_active_unset(label="対応形式確認" tag="nuxt-link" :to="{name: 'adapter-format'}" @click.native="$sound.play_click()")
</template>
