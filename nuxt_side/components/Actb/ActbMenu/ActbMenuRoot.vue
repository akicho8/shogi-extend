...
.ActbMenuRoot
  ...
  .menu_buttons
    b-button(expanded tag="nuxt-link" :to="{name: 'settings-profile'}" :disabled="!base.current_user" @click.native="sound_play_click()") プロフィール
    b-button(expanded @click="base.emotion_index_handle" :disabled="!base.current_user" v-if="base.config.emotion_editable_p") エモーション
    b-button(expanded tag="a" :href="question_zip_download_url" @click="sound_play_click()" :disabled="!base.current_user" v-if="development_p") 問題ダウンロード(直接)
