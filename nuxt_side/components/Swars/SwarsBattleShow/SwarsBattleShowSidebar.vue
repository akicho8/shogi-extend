    ...
    .mt-4
      b-menu
        b-menu-list(label="Action")
          b-menu-item.is_active_unset(label="共有将棋盤に転送"       tag="nuxt-link" :to="{name: 'share-board', query: base.share_board_query}" @click.native="$sound.play_click()")
          b-menu-item.is_active_unset(label="問題作成"               tag="nuxt-link" :to="{name: 'rack-articles-new', query: {body: base.record.sfen_body, turn: base.current_turn, viewpoint: base.viewpoint}}" @click.native="$sound.play_click()")
          b-menu-item.is_active_unset(label="ｽﾀｲﾙｴﾃﾞｨﾀに転送" tag="nuxt-link" :to="{name: 'style-editor', query: base.style_editor_query}" @click.native="$sound.play_click()")
        ...
        SbSidebarExport(:base="base")
          ...
          b-menu-item.is_active_unset(icon="movie"   label="動画変換"       tag="nuxt-link" :to="{name: 'video-new', query: {body: base.record.sfen_body, viewpoint: base.viewpoint}}" @click.native="$sound.play_click()")
