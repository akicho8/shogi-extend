    ...
    .mt-4
      b-menu
        ...
        b-menu-list(label="export")
          ...
          b-menu-item.is_active_unset.menu_item_show(:expanded="false" @click="sound_play_click()")
            template(slot="label" slot-scope="props")
              | 表示
              b-icon.is-pulled-right(:icon="props.expanded ? 'menu-up' : 'menu-down'")
            template(v-for="e in FormatTypeInfo.values")
              template(v-if="e.show")
                b-menu-item.is_active_unset(:class="e.key" :label="e.name" @click="base.sidebar_close" :target="target_default" :href="show_url_for(e.format_key)")

          b-menu-item.is_active_unset.menu_item_copy(@click="sound_play_click()")
            template(slot="label" slot-scope="props")
              | コピー
              b-icon.is-pulled-right(:icon="props.expanded ? 'menu-up' : 'menu-down'")
            template(v-for="e in FormatTypeInfo.values")
              template(v-if="e.clipboard")
                b-menu-item.is_active_unset(:class="e.key" :label="e.name" @click="swars_clipboard_copy_handle(e.format_key)")

          b-menu-item.is_active_unset.menu_item_dl(@click="sound_play_click()")
            template(slot="label" slot-scope="props")
              span.ml-1 ダウンロード
              b-icon.is-pulled-right(:icon="props.expanded ? 'menu-up' : 'menu-down'")
            b-menu-item.is_active_unset(class="kif_utf8" label="KIF"             @click="base.dl_click_handle" :href="dl_url_for('kif', {body_encode: 'UTF-8'})")
            b-menu-item.is_active_unset(                 label="KIF (Shift_JIS)" @click="base.dl_click_handle" :href="dl_url_for('kif', {body_encode: 'Shift_JIS'})")
            b-menu-item.is_active_unset(                 label="KI2"             @click="base.dl_click_handle" :href="dl_url_for('ki2')")
            b-menu-item.is_active_unset(                 label="CSA"             @click="base.dl_click_handle" :href="dl_url_for('csa')")
            b-menu-item.is_active_unset(                 label="SFEN"            @click="base.dl_click_handle" :href="dl_url_for('sfen')")
            b-menu-item.is_active_unset(                 label="BOD"             @click="base.dl_click_handle" :href="dl_url_for('bod', {turn: base.new_turn})")
            b-menu-item.is_active_unset(                 label="PNG"             @click="base.dl_click_handle" :href="dl_url_for('png', {turn: base.new_turn, viewpoint: base.new_viewpoint, width: ''})")
