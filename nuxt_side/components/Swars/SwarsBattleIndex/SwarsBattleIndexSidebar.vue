    ...
    .mt-4
      b-menu
            ...
            b-menu-item.is_active_unset(
              ...
              template(slot="label")
                ...
                template(v-if="e.key === 'is_layout_table'")
                  b-dropdown.is-pulled-right(position="is-bottom-left" :close-on-click="false" :mobile-modal="false" @active-change="false && sfx_click()")
        ...
        b-menu-list(label="オプション")
          b-menu-item.is_active_unset.per_change_menu_item(@click="sfx_click()")
        ...
        b-menu-list(label="まとめて取得")
          b-menu-item.is_active_unset.swars_direct_download_handle(
            label="棋譜ダウンロード"
            @click.native="APP.xi.current_swars_user_key && sfx_click()"
            tag="nuxt-link"
        ...
        b-menu-list(label="一歩進んだ使い方")
          ...
          b-menu-item.is_active_unset.external_app_menu_item(:disabled="menu_item_disabled" @click="sfx_click()")
            template(slot="label" slot-scope="props")
              | 外部アプリですぐ開く
              b-icon.is-pulled-right(:icon="props.expanded ? 'menu-up' : 'menu-down'")
