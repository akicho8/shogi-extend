...
.modal-card
  .modal-card-head
    .modal-card-title
      | 棋譜の読み込み
  .modal-card-body
    b-input(type="textarea" v-model.trim="any_source" ref="any_source" rows="6" placeholder="KIF KI2 CSA SFEN BOD の本体かURL。棋譜をパラメータに含むURL。KENTOや将棋DB2のURL。棋譜URLをコンテンツに含むサイトURL。戦法名・囲い名などを入力してください")
  .modal-card-foot
