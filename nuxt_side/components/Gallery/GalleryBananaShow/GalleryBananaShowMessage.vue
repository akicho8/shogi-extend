<template lang="pug">
.GalleryBananaShowMessage
  .media(v-for="banana_message in base.banana.banana_messages")
    GalleryBananaShowAvatarLinkTo(:base="base" :user="banana_message.user")
    .media-content
      .content
        p
          nuxt-link.user_link.has-text-weight-semibold(:to="{name: 'users-id', params: {id: banana_message.user.id}}" @click.native="sound_play_click()")
  ...
  .media.MessageInput(v-if="base.g_current_user")
    GalleryBananaShowAvatarLinkTo(:base="base" :user="base.g_current_user")
    .media-content
      .field
        .control
