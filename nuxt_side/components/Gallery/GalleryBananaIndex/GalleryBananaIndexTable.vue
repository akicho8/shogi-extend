...
.GalleryBananaIndexTable
  b-table(
    ...
    b-table-column(v-slot="{row}" custom-key="operation" label="" width="1")
      ...
      //-   //- b-icon(icon="edit")
      //-   | 編集
      b-button(tag="nuxt-link" :to="{name: 'video-studio-banana_key-edit', params: {banana_key: row.key}}" size="is-small" @click.native="sound_play('click')") 編集
      ...
      //-     nuxt-link(:to="{name: 'video-studio-banana_key-edit', params: {banana_key: row.key}}" @click.native="sound_play('click')") 編集
      //-
      //-   a(slot="trigger")
      ...
      //-       nuxt-link(:to="{name: 'video-studio-banana_key-edit', params: {banana_key: row.key}}" @click.native="sound_play('click')") 編集
      //-     //- b-dropdown-item(has-link)
      //-     //-   nuxt-link(:to="{name: 'video-articles-new', query: {banana_key: row.key}}"        @click.native="sound_play('click')") 問題追加
