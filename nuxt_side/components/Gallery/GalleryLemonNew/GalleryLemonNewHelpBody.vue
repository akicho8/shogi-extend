<template lang="pug">
.GalleryLemonNewHelpBody.is_line_break_on
  .content
    article
      h2 簡単な使い方
      .article_body
    ...
    article
      ...
      .article_body
        p 登録すると次のことができる
        ol
          li 専用ページを作成してタイトルや説明を追加する
          li みんなに公開してコメントをもらう
          li 仲間内だけで共有する
          li 自分だけでこっそり見返す
        p 作成したファイルを即ダウンロードするだけであれば必要ない
    ...
    article
      ...
      .article_body
        .content
          p
            code 1ページあたりの秒数 * (1 + 手数) + 最後に停止する秒数
            | で動画の長さが求まる。
          p
...
export default {
  name: "GalleryLemonNewHelpBody",
}
</script>

<style lang="sass">
@import "all_support.sass"
.GalleryLemonNewHelpBody
  padding: 1.5rem
  article
    &:not(:first-child)
      margin-top: 2.5rem
