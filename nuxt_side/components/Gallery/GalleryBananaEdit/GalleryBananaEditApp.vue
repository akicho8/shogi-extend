...
client-only
  .GalleryBananaEditApp
    ...
    GalleryBananaEditNavbar(:base="base")

    MainSection.when_mobile_footer_scroll_problem_workaround
      .container.is-fluid
        GalleryBananaEditForm(:base="base" v-if="!$fetchState.pending && !$fetchState.error")

    GalleryBananaEditDebugPanels(:base="base" v-if="development_p")
</template>
...
export default {
  ...
  async fetch() {
    ...
    const e = await this.$axios.$get("/api/gallery/bananas/edit.json", {params})
    // if (e.error) {
  ...
  methods: {
    banana_save_handle() {
      ...
      if (this.blank_p(this.banana.title)) {
      ...
      return this.$axios.$post("/api/gallery/bananas/save.json", {banana: this.banana.post_params}).then(e => {
