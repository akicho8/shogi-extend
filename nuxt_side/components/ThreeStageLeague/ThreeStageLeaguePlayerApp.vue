...
.ThreeStageLeaguePlayerApp
  ...
  MainNavbar
    ...
    template(slot="end")
      b-navbar-item.has-text-weight-bold(tag="a" :href="image_search_url(config.main_user.name)" target="_blank") ググる
        ...
        .column
          ...
          b-table.mt-3(:data="config.memberships" :mobile-cards="false" hoverable)
            b-table-column(v-slot="{row}" field="league.generation" label="期" numeric sortable)
              nuxt-link(:to="{name: 'three-stage-leagues-generation', params: {generation: row.league.generation}}" @click.native="$sound.play_click()")
