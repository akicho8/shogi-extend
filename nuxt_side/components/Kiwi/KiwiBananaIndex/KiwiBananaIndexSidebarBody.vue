...
.KiwiBananaIndexSidebarBody
  b-menu
    ...
    //-   b-menu-item(tag="nuxt-link" :to="{name: 'video-articles'}" label="問題リスト"   @click.native="sfx_click()")
    //-   b-menu-item(tag="nuxt-link" :to="{name: 'video-studio'}"    label="動画リスト" @click.native="sfx_click()")

    b-menu-list(label="表示オプション" v-if="base.visible_hash")
      b-menu-item.is_active_unset.sidebar_columns_toggle(@click="sfx_click()")
