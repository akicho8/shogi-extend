...
.KiwiBananaIndexTable
  b-table(
    ...
    b-table-column(v-slot="{row}" custom-key="title" field="title" :label="base.BananaIndexColumnInfo.fetch('title').name" sortable width="" cell-class="title_column")
      nuxt-link.is_line_break_on(:to="{name: 'video-watch-banana_key', params: {banana_key: row.key}}" @click.native="sound_play_click()")
        | {{row.title}}
    ...
    b-table-column(v-slot="{row}" custom-key="created_at" field="created_at" :label="base.BananaIndexColumnInfo.fetch('created_at').name" sortable) {{time_format_row(row.created_at)}}

    b-table-column(v-slot="{row}" custom-key="updated_at" field="updated_at" :label="base.BananaIndexColumnInfo.fetch('updated_at').name" sortable) {{time_format_row(row.updated_at)}}
    ...
    b-table-column(v-slot="{row}" custom-key="operation" label="" width="1")
      ...
      //-   //- b-icon(icon="edit")
      //-   | 編集
      b-button(tag="nuxt-link" :to="{name: 'video-studio-banana_key-edit', params: {banana_key: row.key}}" size="is-small" @click.native="sound_play_click()") 編集

      //- nuxt-link(:to="{name: 'video-articles-new', query: {banana_key: row.key}}" v-if="false")
      //-   b-icon(icon="plus")

      //- b-dropdown(append-to-body position="is-bottom-left" @active-change="sound_play_click()")
      //-   b-dropdown-item(has-link)
      //-     nuxt-link(:to="{name: 'video-studio-banana_key-edit', params: {banana_key: row.key}}" @click.native="sound_play_click()") 編集
      //-
      //-   a(slot="trigger")
      ...
      //-       nuxt-link(:to="{name: 'video-studio-banana_key-edit', params: {banana_key: row.key}}" @click.native="sound_play_click()") 編集
      //-     //- b-dropdown-item(has-link)
      //-     //-   nuxt-link(:to="{name: 'video-articles-new', query: {banana_key: row.key}}"        @click.native="sound_play_click()") 問題追加
