...
.KiwiBookEditArticleIndexTable
  ...
  b-table.mt-1(
    ...
    b-table-column(v-slot="{row}" custom-key="article.title" field="article.title" sortable label="タイトル" cell-class="is_line_break_on title_column" header-class="title_column")
      nuxt-link.article_title(:to="{name: 'video-articles-article_key', params: {article_key: row.article.key}}" @click.native="sound_play('click')")
        | {{row.article.title}}
...
export default {
  name: "KiwiBookEditArticleIndexTable",
  mixins: [
    support_child,
  ],
  data() {
