export const app_book_room = {
  data() {
    return {
      ac_book_room: null, // subscriptions.create のインスタンス
      ac_book_room_connected_count: 0, // 接続回数
    }
  },
  mounted() {
    // this.book_room_destroy()
  ...
  methods: {
    book_room_create() {
      this.__assert__(this.ac_book_room == null, "this.ac_book_room == null")
      // alert(this.book.id)
      this.ac_book_room = this.ac_subscription_create({channel: "Kiwi::BookRoomChannel", book_id: this.book.id}, {
        connected: e => {
          if (this.ac_book_room_connected_count === 0) {
            this.book_room_connected()
          }
          this.ac_book_room_connected_count += 1
        },
      })
    },

    book_room_connected() {
      this.ac_book_room_perform("setup_request", {})
    },

    book_room_destroy() {
      if (this.ac_book_room) {
        this.ac_unsubscribe("ac_book_room")
      }
    },
    ...
    ac_book_room_perform(action, params = {}) {
      if (this.ac_book_room) {
        this.ac_book_room.perform(action, {
          ...this.ac_book_room_perform_default_params(),
          ...params,
        }) // --> app/channels/kiwi/book_room_channel.rb
      }
    },

    ac_book_room_perform_default_params() {
      const params = {
      }
      return params
    },
    ...
    ac_log(subject = "", body = "") {
      this.ac_book_room_perform("ac_log", { subject, body })
    },

    kiwi_book_message_pong_singlecast(data) {
      this.debug_alert(`singlecast: ${data.pong}`)
    },

    kiwi_book_message_pong_broadcast(data) {
