※development, test は production と同じ
|------+----+------------------------+------------+-------------------------------------|
| host | db |                        | env        | 検索キーワード                      |
|------+----+------------------------+------------+-------------------------------------|
| ik1  |  0 |                        |            |                                     |
|      |  1 | Rails.cache            | production | redis_cache_store                   |
|      |  2 | 符号の鬼のランキング用 | production | redis_db_for_xy_rule_info           |
|      |  3 | 対戦のランキング用     | production | redis_db_for_colosseum_ranking_info |
|      |  4 | 詰将棋ファイター(旧)   | production | redis_db_for_acns2                  |
|      |  5 | 詰将棋ファイター(新)   | production | redis_db_for_acns3                  |
|      |  6 | sidekiq                | production | redis_db_for_sidekiq                |
|      |  7 | ActionCable            | production | cable.yml                           |
|      |  8 |                        |            |                                     |
|      |  9 |                        |            |                                     |
|      | 10 |                        |            |                                     |
|      | 11 |                        |            |                                     |
|      | 12 |                        |            |                                     |
|      | 13 |                        |            |                                     |
|      | 14 |                        |            |                                     |
|      | 15 |                        |            |                                     |
|------+----+------------------------+------------+-------------------------------------|
| tk2  |  0 |                        |            |                                     |
|      |  1 |                        |            |                                     |
|      |  2 |                        |            |                                     |
|      |  3 |                        |            |                                     |
|      |  4 |                        |            |                                     |
|      |  5 |                        |            |                                     |
|      |  6 |                        |            |                                     |
|      |  7 |                        |            |                                     |
|      |  8 | Rails.cache            | staging    | redis_cache_store                   |
|      |  9 | 符号の鬼のランキング用 | staging    | redis_db_for_xy_rule_info           |
|      | 10 | 対戦のランキング用     | staging    | redis_db_for_colosseum_ranking_info |
|      | 11 | 詰将棋ファイター(旧)   | staging    | redis_db_for_acns2                  |
|      | 12 | 詰将棋ファイター(新)   | staging    | redis_db_for_acns3                  |
|      | 13 | sidekiq                | staging    | redis_db_for_sidekiq                |
|      | 14 | ActionCable            | staging    | cable.yml                           |
|      | 15 |                        |            |                                     |
|------+----+------------------------+------------+-------------------------------------|
